...
# -*- coding: utf-8 -*-
r"""
Phase 41: Counterfactual holdout-context causal abstraction verifier
...
Phase 41 adds a stricter verification problem: the system now receives several source
...
Outputs:
  E:\BBIT\outputs_basic32\phase41_holdout_context_verifier_trials.csv
  E:\BBIT\outputs_basic32\phase41_holdout_context_verifier_summary.json
  E:\BBIT\outputs_basic32\phase41_holdout_context_verifier_report.md
  E:\BBIT\outputs_basic32\phase41_holdout_poison_sweep.csv
  E:\BBIT\outputs_basic32\phase41_examples\*.png
...
PHASE = "41"
TITLE = "Counterfactual holdout-context causal abstraction verifier"
SCRIPT_NAME = "geomlang_phase40_counterfactual_twin_context_causal_abstraction_reasoner_basic32_E_drive.py"
ROOT = Path(r"E:\BBIT") if Path(r"E:\BBIT").exists() else Path.cwd()
OUT = ROOT / "outputs_basic32"
EXAMPLE_DIR = OUT / "phase41_examples"
...
def make_twin_candidates(
    ...
    # Looks good in the first target context, breaks under the holdout-context replay.
...
def plot_accuracy(summary: Dict[str, object]) -> None:
    ...
    ax.set_title("Phase 41 counterfactual holdout-context causal abstraction reasoning: accuracy by hidden family")
    for i, v in enumerate(vals):
        ax.text(v + 0.01, i, f"{v:.2f}", va="center")
    fig.tight_layout()
    fig.savefig(OUT / "phase41_holdout_context_verifier_accuracy.png", dpi=140)
    plt.close(fig)
...
def plot_baseline_gap(summary: Dict[str, object]) -> None:
    ...
    ax.bar(x + width/2, traj, width, label="holdout-context verifier")
    ax.set_ylim(0, 1.05)
    ax.set_ylabel("accuracy")
    ax.set_title("Phase 41 final-only baseline vs holdout-context causal abstraction")
    ax.set_xticks(x)
    ax.set_xticklabels(labels, rotation=22, ha="right")
    ax.legend()
    fig.tight_layout()
    fig.savefig(OUT / "phase41_holdout_context_verifier_baseline_gap.png", dpi=140)
    plt.close(fig)
...
def plot_branch_counts(rows: List[Dict[str, object]]) -> None:
    ...
    ax.bar(labels, vals)
    ax.set_ylabel("trials")
    ax.set_title("Phase 41 predicted branch counts")
    ax.tick_params(axis="x", rotation=25)
    fig.tight_layout()
    fig.savefig(OUT / "phase41_holdout_context_verifier_branch_counts.png", dpi=140)
    plt.close(fig)
...
def write_poison_sweep(rows: List[Dict[str, float]]) -> None:
    path = OUT / "phase41_holdout_poison_sweep.csv"
    with path.open("w", newline="", encoding="utf-8") as f:
    ...
    ax.set_ylabel("accuracy")
    ax.set_title("Phase 41 poisoned-witness stress sweep")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(OUT / "phase41_holdout_context_verifier_poison_sweep.png", dpi=140)
    plt.close(fig)
...
def write_report(summary: Dict[str, object], path: Path) -> None:
    ...
    lines.append("\n## Interpretation\n")
    lines.append(
        "Phase 41 tests whether the path representation can become a consensus object. "
...
def save_examples(rng: np.random.Generator, n: int = 4) -> None:
    ...
    for i, fam in enumerate(FAMILIES[:n]):
        ...
        axes[2].set_title("holdout-context rejection")
        axes[2].axis("equal")
        axes[2].legend(fontsize=8)
        fig.tight_layout()
        fig.savefig(EXAMPLE_DIR / f"phase40_example_{i+1}_{fam}.png", dpi=140)
        plt.close(fig)
...
def main() -> None:
    ...
    print(f"[{PHASE}] reset continued: from cross-context bridge reasoning to holdout-context causal abstraction")
    print(f"[{PHASE}] task: infer invariant A->B->C from witnesses, then require the answer branch to survive a counterfactual holdout-context replay")

    rows = [run_trial(rng, i, poison_fraction=POISON_FRACTION) for i in range(TRIALS)]
    summary = aggregate(rows)

    trials_path = OUT / "phase41_holdout_context_verifier_trials.csv"
    summary_path = OUT / "phase41_holdout_context_verifier_summary.json"
    report_path = OUT / "phase41_holdout_context_verifier_report.md"
    ...
    plot_hist(rows, "margin", "Phase 41 holdout-context trajectory answer margin distribution", "runner-up trajectory score - best trajectory score", "phase41_holdout_context_verifier_margin_distribution.png")
    plot_hist(rows, "final_margin", "Phase 41 final-only endpoint margin distribution", "runner-up endpoint score - best endpoint score", "phase41_holdout_context_verifier_final_only_margin_distribution.png")
    sweep_rows = poison_sweep()
    write_poison_sweep(sweep_rows)
    save_examples(rng)

    print(f"[{PHASE}] PHASE41_COUNTERFACTUAL_HOLDOUT_CONTEXT_CAUSAL_ABSTRACTION_PASS={summary['pass']}")
    print(
    ...
    print(f"[{PHASE}] wrote holdout poison sweep: {OUT / 'phase41_holdout_poison_sweep.csv'}")
