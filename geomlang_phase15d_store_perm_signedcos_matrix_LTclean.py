#!/usr/bin/env python3
# geomlang_phase15d_store_perm_signedcos_matrix_LTclean.py
#
# Phase 15D: Same as 15C, but CLEAN the LT plane.
#
# Adds:
#  - v_lr_clean / v_tproj_clean:
#     1) remove components along BO (between/overlap clean)
#     2) then make lr and tproj mutually clean
#
# Still stores:
#   cos2x2_raw = (Qt^T QB)  (signed)
#   cos2x2_abs = abs(cos2x2_raw)
#
# Output: outputs_edges_relternary256_phase15/phase15d_transport_cache.json
...
def gram_schmidt_clean(v_main, v_remove):
    v_remove_u = unit(v_remove)
    coeff = float(np.dot(v_main, v_remove_u))
    v_clean = v_main - coeff * v_remove_u
...
def main():
    ...
    for name, idx in anchors.items():
        ...
        # --- BO cleaning (same as 15c) ---
        vb_clean, cb = gram_schmidt_clean(vb, vo)
        vo_clean, co = gram_schmidt_clean(vo, vb)

        # --- LT cleaning (new in 15d) ---
        # 1) remove BO components from lr and tproj
        vl1, c_l_on_b = gram_schmidt_clean(vl, vb_clean)
        vl2, c_l_on_o = gram_schmidt_clean(vl1, vo_clean)

        vt1, c_t_on_b = gram_schmidt_clean(vt, vb_clean)
        vt2, c_t_on_o = gram_schmidt_clean(vt1, vo_clean)

        # 2) make lr and tproj mutually clean (stable LT axes)
        vl_clean, c_l_on_t = gram_schmidt_clean(vl2, vt2)
        vt_clean, c_t_on_l = gram_schmidt_clean(vt2, vl_clean)
        ...
        meta[name] = {
            ...
            "clean_coeffs": {
                ...
                "lr_on_between": c_l_on_b,
                "lr_on_overlap": c_l_on_o,
                "tproj_on_between": c_t_on_b,
                "tproj_on_overlap": c_t_on_o,
                "lr_on_tproj": c_l_on_t,
                "tproj_on_lr": c_t_on_l,
            },
            "signals_at_seed": {
    ...
    for a in names:
        ...
        QA_lt = ortho2(bases[a]["v_lr_clean"],     bases[a]["v_tproj_clean"])
    ...
    out = {
        ...
        "notes": [
            "Phase15d cleans LT: lr/tproj are cleaned against BO, then mutually cleaned.",
    ...
    out_path = os.path.join(OUTDIR, "phase15d_transport_cache.json")
    with open(out_path, "w") as f:
        json.dump(out, f, indent=2)

    print("[phase15d] saved ->", out_path)
