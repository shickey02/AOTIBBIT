...
# -*- coding: utf-8 -*-
r"""
Phase 44: Counterfactual adversarial-coalition causal resilience verifier
...
Phase 44 adds a stricter necessity-verification problem: the system now receives several source
...
Task:
  ...
  Phase 44 adds a adversarial-coalition necessity check: the consensus is recomputed
...
Outputs:
  E:\BBIT\outputs_basic32\phase44_adversarial_coalition_verifier_trials.csv
  E:\BBIT\outputs_basic32\phase44_adversarial_coalition_verifier_summary.json
  E:\BBIT\outputs_basic32\phase44_adversarial_coalition_verifier_report.md
  E:\BBIT\outputs_basic32\phase44_adversarial_coalition_poison_sweep.csv
  E:\BBIT\outputs_basic32\phase44_examples\*.png
...
PHASE = "44"
TITLE = "Counterfactual adversarial-coalition causal resilience verifier"
SCRIPT_NAME = "geomlang_phase44_counterfactual_adversarial_coalition_causal_sufficiency_verifier_basic32_E_drive.py"
ROOT = Path(r"E:\BBIT") if Path(r"E:\BBIT").exists() else Path.cwd()
OUT = ROOT / "outputs_basic32"
EXAMPLE_DIR = OUT / "phase44_examples"
...
def infer_consensus(witnesses: List[Tuple[np.ndarray, np.ndarray, np.ndarray, bool]]) -> Tuple[Affine, Affine, Dict[str, float]]:
    ...
    # Phase 44 adversarial-coalition sufficiency diagnostic: rebuild the weighted-median
    ...
    # Phase 44 adversarial-coalition diagnostic: now ask a harder question.
    ...
    return c1, c2, {
        ...
        "adversarial_coalition_error": float(np.mean(coalition_drifts)),
        "adversarial_coalition_max_error": float(np.max(coalition_drifts)),
        "adversarial_coalition_best_error": float(np.min(coalition_drifts)),
        "adversarial_coalition_size": float(coalition_size),
        "adversarial_coalition_mean_fit": float(np.mean(coalition_scores)),
...
def aggregate(rows: List[Dict[str, object]]) -> Dict[str, object]:
    ...
    summary: Dict[str, object] = {
        ...
        "mean_adversarial_coalition_error": float(arr("adversarial_coalition_error").mean()),
        "mean_adversarial_coalition_max_error": float(arr("adversarial_coalition_max_error").mean()),
        "mean_adversarial_coalition_best_error": float(arr("adversarial_coalition_best_error").mean()),
        "mean_adversarial_coalition_size": float(arr("adversarial_coalition_size").mean()),
        "mean_adversarial_coalition_fit": float(arr("adversarial_coalition_mean_fit").mean()),
    ...
    for fam in FAMILIES:
        ...
        fams[fam] = {
            ...
            "mean_adversarial_coalition_error": float(aa("adversarial_coalition_error").mean()),
...
def plot_accuracy(summary: Dict[str, object]) -> None:
    ...
    ax.set_title("Phase 44 counterfactual adversarial-coalition causal resilience reasoning: accuracy by hidden family")
    for i, v in enumerate(vals):
        ax.text(v + 0.01, i, f"{v:.2f}", va="center")
    fig.tight_layout()
    fig.savefig(OUT / "phase44_adversarial_coalition_verifier_accuracy.png", dpi=140)
    plt.close(fig)
...
def plot_baseline_gap(summary: Dict[str, object]) -> None:
    ...
    ax.bar(x + width/2, traj, width, label="adversarial-coalition verifier")
    ax.set_ylim(0, 1.05)
    ax.set_ylabel("accuracy")
    ax.set_title("Phase 44 final-only baseline vs adversarial-coalition causal resilience")
    ax.set_xticks(x)
    ax.set_xticklabels(labels, rotation=22, ha="right")
    ax.legend()
    fig.tight_layout()
    fig.savefig(OUT / "phase44_adversarial_coalition_verifier_baseline_gap.png", dpi=140)
    plt.close(fig)
...
def plot_branch_counts(rows: List[Dict[str, object]]) -> None:
    ...
    ax.bar(labels, vals)
    ax.set_ylabel("trials")
    ax.set_title("Phase 44 predicted branch counts")
    ax.tick_params(axis="x", rotation=25)
    fig.tight_layout()
    fig.savefig(OUT / "phase44_adversarial_coalition_verifier_branch_counts.png", dpi=140)
    plt.close(fig)
...
def write_poison_sweep(rows: List[Dict[str, float]]) -> None:
    path = OUT / "phase44_adversarial_coalition_poison_sweep.csv"
    with path.open("w", newline="", encoding="utf-8") as f:
    ...
    ax.set_ylabel("accuracy")
    ax.set_title("Phase 44 poisoned-witness stress sweep")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(OUT / "phase44_adversarial_coalition_verifier_poison_sweep.png", dpi=140)
    plt.close(fig)
...
def write_report(summary: Dict[str, object], path: Path) -> None:
    ...
    lines.append(f"- mean adversarial-coalition error: `{summary['mean_adversarial_coalition_error']:.6f}`")
    lines.append(f"- mean adversarial-coalition max error: `{summary['mean_adversarial_coalition_max_error']:.6f}`")
    lines.append(f"- mean adversarial-coalition best error: `{summary['mean_adversarial_coalition_best_error']:.6f}`")
    lines.append(f"- mean adversarial-coalition size: `{summary['mean_adversarial_coalition_size']:.1f}`")
    ...
    lines.append("\n## Interpretation\n")
    lines.append(
        "Phase 44 tests whether the path representation is sufficient when reconstructed from small counterfactual quorums rather than the whole witness pool. "
        ...
        "the robustly inferred two-step causal path across many minimal 3-of-5 witness quorums."
    )
...
def save_examples(rng: np.random.Generator, n: int = 4) -> None:
    ...
    for i, fam in enumerate(FAMILIES[:n]):
        ...
        axes[2].set_title("adversarial-coalition rejection")
        axes[2].axis("equal")
        axes[2].legend(fontsize=8)
        fig.tight_layout()
        fig.savefig(EXAMPLE_DIR / f"phase44_example_{i+1}_{fam}.png", dpi=140)
        plt.close(fig)
...
def main() -> None:
    ...
    print(f"[{PHASE}] reset continued: from leave-one necessity to adversarial-coalition causal resilience")
    print(f"[{PHASE}] task: infer invariant A->B->C from witnesses, then require the answer branch to survive multiple adversarial coalition removal and minimal quorum replays")

    rows = [run_trial(rng, i, poison_fraction=POISON_FRACTION) for i in range(TRIALS)]
    summary = aggregate(rows)

    trials_path = OUT / "phase44_adversarial_coalition_verifier_trials.csv"
    summary_path = OUT / "phase44_adversarial_coalition_verifier_summary.json"
    report_path = OUT / "phase44_adversarial_coalition_verifier_report.md"
    ...
    plot_hist(rows, "margin", "Phase 44 adversarial-coalition trajectory answer margin distribution", "runner-up trajectory score - best trajectory score", "phase44_adversarial_coalition_verifier_margin_distribution.png")
    plot_hist(rows, "final_margin", "Phase 44 final-only endpoint margin distribution", "runner-up endpoint score - best endpoint score", "phase44_adversarial_coalition_verifier_final_only_margin_distribution.png")
    sweep_rows = poison_sweep()
    write_poison_sweep(sweep_rows)
    save_examples(rng)

    print(f"[{PHASE}] PHASE44_COUNTERFACTUAL_ADVERSARIAL_COALITION_CAUSAL_RESILIENCE_PASS={summary['pass']}")
    print(
    ...
    print(
        ...
        f"mean_adversarial_coalition_error={summary['mean_adversarial_coalition_error']:.6f} "
        f"mean_adversarial_coalition_max_error={summary['mean_adversarial_coalition_max_error']:.6f} "
        f"mean_adversarial_coalition_best_error={summary['mean_adversarial_coalition_best_error']:.6f} "
    ...
    print(f"[{PHASE}] wrote adversarial-coalition poison sweep: {OUT / 'phase44_adversarial_coalition_poison_sweep.csv'}")
