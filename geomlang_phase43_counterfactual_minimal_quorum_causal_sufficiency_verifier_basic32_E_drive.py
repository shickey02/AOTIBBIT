...
# -*- coding: utf-8 -*-
r"""
Phase 43: Counterfactual minimal-quorum causal sufficiency verifier
...
Phase 43 adds a stricter necessity-verification problem: the system now receives several source
...
Task:
  ...
  Phase 43 adds a minimal-quorum necessity check: the consensus is recomputed
...
Outputs:
  E:\BBIT\outputs_basic32\phase43_minimal_quorum_verifier_trials.csv
  E:\BBIT\outputs_basic32\phase43_minimal_quorum_verifier_summary.json
  E:\BBIT\outputs_basic32\phase43_minimal_quorum_verifier_report.md
  E:\BBIT\outputs_basic32\phase43_minimal_quorum_poison_sweep.csv
  E:\BBIT\outputs_basic32\phase43_examples\*.png
...
PHASE = "43"
TITLE = "Counterfactual minimal-quorum causal sufficiency verifier"
SCRIPT_NAME = "geomlang_phase43_counterfactual_minimal_quorum_causal_sufficiency_verifier_basic32_E_drive.py"
ROOT = Path(r"E:\BBIT") if Path(r"E:\BBIT").exists() else Path.cwd()
OUT = ROOT / "outputs_basic32"
EXAMPLE_DIR = OUT / "phase43_examples"
...
def infer_consensus(witnesses: List[Tuple[np.ndarray, np.ndarray, np.ndarray, bool]]) -> Tuple[Affine, Affine, Dict[str, float]]:
    ...
    # Phase 43 minimal-quorum sufficiency diagnostic: rebuild the weighted-median
...
def aggregate(rows: List[Dict[str, object]]) -> Dict[str, object]:
    ...
    summary: Dict[str, object] = {
        ...
        "mean_minimal_quorum_error": float(arr("minimal_quorum_error").mean()),
        "mean_minimal_quorum_max_error": float(arr("minimal_quorum_max_error").mean()),
        "mean_minimal_quorum_best_error": float(arr("minimal_quorum_best_error").mean()),
        "mean_minimal_quorum_size": float(arr("minimal_quorum_size").mean()),
        "mean_minimal_quorum_fit": float(arr("minimal_quorum_mean_fit").mean()),
    ...
    for fam in FAMILIES:
        ...
        fams[fam] = {
            ...
            "mean_minimal_quorum_error": float(aa("minimal_quorum_error").mean()),
...
def plot_accuracy(summary: Dict[str, object]) -> None:
    ...
    ax.set_title("Phase 43 counterfactual minimal-quorum causal sufficiency reasoning: accuracy by hidden family")
    for i, v in enumerate(vals):
        ax.text(v + 0.01, i, f"{v:.2f}", va="center")
    fig.tight_layout()
    fig.savefig(OUT / "phase43_minimal_quorum_verifier_accuracy.png", dpi=140)
    plt.close(fig)
...
def plot_baseline_gap(summary: Dict[str, object]) -> None:
    ...
    ax.bar(x + width/2, traj, width, label="minimal-quorum verifier")
    ax.set_ylim(0, 1.05)
    ax.set_ylabel("accuracy")
    ax.set_title("Phase 43 final-only baseline vs minimal-quorum causal sufficiency")
    ax.set_xticks(x)
    ax.set_xticklabels(labels, rotation=22, ha="right")
    ax.legend()
    fig.tight_layout()
    fig.savefig(OUT / "phase43_minimal_quorum_verifier_baseline_gap.png", dpi=140)
    plt.close(fig)
...
def plot_branch_counts(rows: List[Dict[str, object]]) -> None:
    ...
    ax.bar(labels, vals)
    ax.set_ylabel("trials")
    ax.set_title("Phase 43 predicted branch counts")
    ax.tick_params(axis="x", rotation=25)
    fig.tight_layout()
    fig.savefig(OUT / "phase43_minimal_quorum_verifier_branch_counts.png", dpi=140)
    plt.close(fig)
...
def write_poison_sweep(rows: List[Dict[str, float]]) -> None:
    path = OUT / "phase43_minimal_quorum_poison_sweep.csv"
    with path.open("w", newline="", encoding="utf-8") as f:
    ...
    ax.set_ylabel("accuracy")
    ax.set_title("Phase 43 poisoned-witness stress sweep")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(OUT / "phase43_minimal_quorum_verifier_poison_sweep.png", dpi=140)
    plt.close(fig)
...
def write_report(summary: Dict[str, object], path: Path) -> None:
    ...
    lines.append(f"- mean minimal-quorum error: `{summary['mean_minimal_quorum_error']:.6f}`")
    lines.append(f"- mean minimal-quorum max error: `{summary['mean_minimal_quorum_max_error']:.6f}`")
    lines.append(f"- mean minimal-quorum best error: `{summary['mean_minimal_quorum_best_error']:.6f}`")
    lines.append(f"- mean minimal-quorum size: `{summary['mean_minimal_quorum_size']:.1f}`")
    ...
    lines.append("\n## Interpretation\n")
    lines.append(
        "Phase 43 tests whether the path representation is sufficient when reconstructed from small counterfactual quorums rather than the whole witness pool. "
...
def save_examples(rng: np.random.Generator, n: int = 4) -> None:
    ...
    for i, fam in enumerate(FAMILIES[:n]):
        ...
        axes[2].set_title("minimal-quorum rejection")
        axes[2].axis("equal")
        axes[2].legend(fontsize=8)
        fig.tight_layout()
        fig.savefig(EXAMPLE_DIR / f"phase43_example_{i+1}_{fam}.png", dpi=140)
        plt.close(fig)
...
def main() -> None:
    ...
    print(f"[{PHASE}] reset continued: from leave-one necessity to minimal-quorum causal sufficiency")
    print(f"[{PHASE}] task: infer invariant A->B->C from witnesses, then require the answer branch to survive multiple minimal witness quorum replays")

    rows = [run_trial(rng, i, poison_fraction=POISON_FRACTION) for i in range(TRIALS)]
    summary = aggregate(rows)

    trials_path = OUT / "phase43_minimal_quorum_verifier_trials.csv"
    summary_path = OUT / "phase43_minimal_quorum_verifier_summary.json"
    report_path = OUT / "phase43_minimal_quorum_verifier_report.md"
    ...
    plot_hist(rows, "margin", "Phase 43 minimal-quorum trajectory answer margin distribution", "runner-up trajectory score - best trajectory score", "phase43_minimal_quorum_verifier_margin_distribution.png")
    plot_hist(rows, "final_margin", "Phase 43 final-only endpoint margin distribution", "runner-up endpoint score - best endpoint score", "phase43_minimal_quorum_verifier_final_only_margin_distribution.png")
    sweep_rows = poison_sweep()
    write_poison_sweep(sweep_rows)
    save_examples(rng)

    print(f"[{PHASE}] PHASE43_COUNTERFACTUAL_MINIMAL_QUORUM_CAUSAL_SUFFICIENCY_PASS={summary['pass']}")
    print(
    ...
    print(
        ...
        f"mean_minimal_quorum_error={summary['mean_minimal_quorum_error']:.6f} "
        f"mean_minimal_quorum_max_error={summary['mean_minimal_quorum_max_error']:.6f} "
        f"mean_minimal_quorum_best_error={summary['mean_minimal_quorum_best_error']:.6f} "
    ...
    print(f"[{PHASE}] wrote minimal-quorum poison sweep: {OUT / 'phase43_minimal_quorum_poison_sweep.csv'}")
