...
# -*- coding: utf-8 -*-
r"""
Phase 37: Intervention-invariant causal path reasoner
...
Phase 37 adds an intervention: part of the observed source middle state is corrupted by
an external force. The reasoner must infer the latent A->B->C causal path anyway, transfer
that latent path to D, and reject branches that copy the intervention artifact or merely
match the final endpoint.
...
Task:
  ...
  Given D and several D->E->F candidate branches, choose the branch preserving the latent
  path, not the branch with the nearest endpoint and not the branch with the copied
  intervention artifact.

Outputs:
  E:\BBIT\outputs_basic32\phase37_intervention_invariant_trials.csv
  E:\BBIT\outputs_basic32\phase37_intervention_invariant_summary.json
  E:\BBIT\outputs_basic32\phase37_intervention_invariant_report.md
  E:\BBIT\outputs_basic32\phase37_intervention_sweep.csv
  E:\BBIT\outputs_basic32\phase37_examples\*.png
...
PHASE = "37"
TITLE = "Intervention-invariant causal path reasoner"
SCRIPT_NAME = "geomlang_phase37_intervention_invariant_causal_path_reasoner_basic32_E_drive.py"
ROOT = Path(r"E:\BBIT") if Path(r"E:\BBIT").exists() else Path.cwd()
OUT = ROOT / "outputs_basic32"
EXAMPLE_DIR = OUT / "phase37_examples"
...
def write_report(path: Path, summary: Dict[str, object]) -> None:
    ...
    lines = []
    lines.append("# Phase 37: Intervention-invariant causal path reasoner\n")
    lines.append("## Intent\n")
    lines.append("Phase 37 tests whether the system can separate a causal path from an external intervention artifact. The observed middle state is partially corrupted, so a final-point or artifact-copy strategy should fail.\n")
    ...
    lines.append("\n## Interpretation\n")
    lines.append("If this phase passes, the reasoner is no longer merely reconstructing noisy paths. It is distinguishing latent causal structure from an intervention that appears inside the observation itself. The correct answer is the branch that preserves the hidden causal path, while distractors preserve only endpoint closeness, the copied intervention, the first step, the second step, or an anti-causal inversion.\n")
...
def plot_accuracy(summary: Dict[str, object]) -> None:
    ...
    ax.set_title("Phase 37 intervention-invariant causal path reasoning: accuracy by hidden family")
    for yi, v in zip(y, vals):
        ax.text(v + 0.01, yi, f"{v:.2f}", va="center")
    fig.tight_layout()
    fig.savefig(OUT / "phase37_intervention_invariant_accuracy.png", dpi=150)
    plt.close(fig)
...
def plot_baseline_gap(summary: Dict[str, object]) -> None:
    ...
    ax.bar(x + width / 2, traj, width, label="intervention-invariant trajectory")
    ax.set_xticks(x)
    ax.set_xticklabels(labels, rotation=25, ha="right")
    ax.set_ylim(0, 1.05)
    ax.set_ylabel("accuracy")
    ax.set_title("Phase 37 final-only baseline vs intervention-invariant trajectory reasoning")
    ax.legend()
    fig.tight_layout()
    fig.savefig(OUT / "phase37_intervention_invariant_baseline_gap.png", dpi=150)
    plt.close(fig)
...
def plot_margins(rows: List[Dict[str, object]]) -> None:
    ...
    ax.set_title("Phase 37 intervention-invariant trajectory answer margin distribution")
    ax.set_xlabel("runner-up trajectory score - best trajectory score")
    ax.set_ylabel("trials")
    fig.tight_layout()
    fig.savefig(OUT / "phase37_intervention_invariant_margin_distribution.png", dpi=150)
    plt.close(fig)

    fig, ax = plt.subplots(figsize=(12, 6))
    ax.hist([r["final_only_margin"] for r in rows], bins=36)
    ax.set_title("Phase 37 final-only endpoint margin distribution")
    ax.set_xlabel("runner-up endpoint score - best endpoint score")
    ax.set_ylabel("trials")
    fig.tight_layout()
    fig.savefig(OUT / "phase37_intervention_invariant_final_only_margin_distribution.png", dpi=150)
    plt.close(fig)
...
def plot_branch_counts(summary: Dict[str, object]) -> None:
    ...
    ax.bar(labels, vals)
    ax.set_title("Phase 37 predicted branch counts")
    ax.set_ylabel("trials")
    ax.set_xticks(np.arange(len(labels)))
    ax.set_xticklabels(labels, rotation=25, ha="right")
    fig.tight_layout()
    fig.savefig(OUT / "phase37_intervention_invariant_branch_counts.png", dpi=150)
    plt.close(fig)
...
def intervention_sweep() -> List[Dict[str, float]]:
    ...
    with (OUT / "phase37_intervention_sweep.csv").open("w", newline="", encoding="utf-8") as f:
    ...
    ax.set_ylabel("accuracy")
    ax.set_title("Phase 37 intervention stress sweep")
    ax.grid(True, alpha=0.25)
    ax.legend()
    fig.tight_layout()
    fig.savefig(OUT / "phase37_intervention_invariant_intervention_sweep.png", dpi=150)
    plt.close(fig)
    return rows
...
def plot_example(row: Dict[str, object], idx: int) -> None:
    ...
    for ax in axes.ravel():
        ...
        ax.set_xticks([])
        ax.set_yticks([])
    fig.suptitle(f"Phase 37 example {idx}: {row['family']} | pred={row['pred_branch']} | final-only={row['final_only_pred_branch']}")
    fig.tight_layout()
...
def main() -> None:
    ...
    print(f"[{PHASE}] reset continued: from noisy causal trajectories to intervention-invariant path reconstruction")
    print(f"[{PHASE}] task: infer latent A->B->C despite an intervened middle state, transfer to D, reject artifact/final-only branches")

    rows = [run_trial(rng, i) for i in range(TRIALS)]
    summary = aggregate(rows)

    write_csv(OUT / "phase37_intervention_invariant_trials.csv", rows)
    (OUT / "phase37_intervention_invariant_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    write_report(OUT / "phase37_intervention_invariant_report.md", summary)
    ...
    print(f"[{PHASE}] PHASE37_INTERVENTION_INVARIANT_CAUSAL_PATH_REASONING_PASS={summary['pass']}")
    print(
    ...
    print(f"[{PHASE}] wrote trials: {OUT / 'phase37_intervention_invariant_trials.csv'}")
    print(f"[{PHASE}] wrote summary: {OUT / 'phase37_intervention_invariant_summary.json'}")
    print(f"[{PHASE}] wrote report: {OUT / 'phase37_intervention_invariant_report.md'}")
    print(f"[{PHASE}] wrote intervention sweep: {OUT / 'phase37_intervention_sweep.csv'}")
