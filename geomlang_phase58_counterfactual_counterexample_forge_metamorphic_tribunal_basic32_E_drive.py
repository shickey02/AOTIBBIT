...
# -*- coding: utf-8 -*-
r"""
Phase 58: Counterfactual counterexample-forge metamorphic tribunal

Drop-in path:
  E:\BBIT\bbit_geomlang\geomlang_phase58_counterfactual_counterexample_forge_metamorphic_tribunal_basic32_E_drive.py

Run:
  python bbit_geomlang/geomlang_phase58_counterfactual_counterexample_forge_metamorphic_tribunal_basic32_E_drive.py

Purpose
-------
Phase 51 proved the typed contradiction tribunal could accept the true A -> B -> C certificate,
reject the false certificates, and pass one sealed holdout/null-ablation checks. Phase 58 raises the bar by
...
Phase 58 adds a metamorphic-shadow invariance tribunal on top of the Phase 54 null-ablation near-miss tribunal.
...
Writes into E:\BBIT\outputs_basic32 by default:
  ...
  phase58_counterexample_forge_metamorphic_tribunal_trials.csv
  phase58_counterexample_forge_metamorphic_tribunal_summary.json
  phase58_counterexample_forge_metamorphic_tribunal_report.md
  phase58_counterexample_forge_metamorphic_tribunal_poison_sweep.csv
  phase58_examples\*.png
  phase58_counterexample_forge_metamorphic_tribunal_accuracy.png
  phase58_counterexample_forge_metamorphic_tribunal_rejection_rates.png
  phase58_counterexample_forge_metamorphic_tribunal_false_certificate_score_distribution.png
  phase58_counterexample_forge_metamorphic_tribunal_true_vs_false_score_margin.png
  phase58_counterexample_forge_metamorphic_tribunal_branch_counts.png
  phase58_counterexample_forge_metamorphic_tribunal_poison_sweep.png
  phase58_counterexample_forge_metamorphic_tribunal_forge_rates.png
...
try:
    ...
    plt = None
    print("[58] WARNING: matplotlib unavailable; png plots will be skipped:", exc)


PHASE = "58"
TITLE = "Counterfactual counterexample-forge metamorphic tribunal"
PASS_FLAG = "PHASE58_COUNTERFACTUAL_COUNTEREXAMPLE_FORGE_METAMORPHIC_TRIBUNAL_PASS"
...
def run_trial(
    ...
    # Phase 58 calibration layer.  Phase 52 proved that false certificates can be
    ...
    # Phase 58: metamorphic-shadow invariance tribunal.
    ...
    # lightweight: Phase 58 should not multiply the full certificate grid again; it
    ...
    # is ablated.  Phase 58 asks a harder structural question: does the same
...
def summarize(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # Phase 58 adds a jackknife/bootstrap shadow jury: the certificate must remain good when
...
def write_report(path: Path, summary: Dict[str, Any]) -> None:
    ...
    lines.append("")
    lines.append(
        "Phase 58 is a falsification phase. Earlier phases proved that the same branch can survive "
...
def plot_outputs(out_dir: Path, rows: List[Dict[str, Any]], summary: Dict[str, Any]) -> None:
    ...
    plt.ylim(0, 1.08)
    plt.ylabel("rate")
    plt.title("Phase 58 final-only vs true-certificate acceptance and false-certificate rejection")
    plt.legend()
    savefig("phase58_counterexample_forge_metamorphic_tribunal_accuracy.png")
    ...
    plt.title("Phase 58 closest false certificate type")
    savefig("phase58_counterexample_forge_metamorphic_tribunal_rejection_rates.png")
    ...
    plt.ylabel("count")
    plt.title("Phase 58 false certificate score distribution")
    savefig("phase58_counterexample_forge_metamorphic_tribunal_false_certificate_score_distribution.png")
    ...
    plt.ylabel("trials")
    plt.title("Phase 58 true-vs-false certificate selectivity margin")
    savefig("phase58_counterexample_forge_metamorphic_tribunal_true_vs_false_score_margin.png")
    ...
    plt.ylabel("trials")
    plt.title("Phase 58 calibrated near-miss selectivity margin")
    savefig("phase58_counterexample_forge_metamorphic_tribunal_near_miss_margin.png")
    ...
    plt.ylabel("count")
    plt.title("Phase 58 log-scaled false certificate score distribution")
    savefig("phase58_counterexample_forge_metamorphic_tribunal_false_certificate_log_score_distribution.png")
    ...
    plt.title("Phase 58 sealed holdout tribunal pass rate by family")
    savefig("phase58_counterexample_forge_metamorphic_tribunal_holdout_pass_rate.png")
    ...
    plt.xticks(rotation=25, ha="right")
    plt.ylabel("trials")
    plt.title("Phase 58 predicted branch counts")
    savefig("phase58_counterexample_forge_metamorphic_tribunal_branch_counts.png")

    # Phase 58 cross-exam rates: jackknife, bootstrap, and pair panels.
    ...
    plt.ylim(0, 1.08)
    plt.ylabel("rate")
    plt.title("Phase 58 jackknife/bootstrap/pair cross-exam consensus")
    plt.legend()
    savefig("phase58_counterexample_forge_metamorphic_tribunal_cross_exam_rates.png")
...
def poison_sweep(args: argparse.Namespace, out_dir: Path) -> List[Dict[str, Any]]:
    ...
    write_csv(out_dir / "phase58_counterexample_forge_metamorphic_tribunal_poison_sweep.csv", sweep_rows)
    ...
    if plt is not None:
        ...
        plt.ylabel("rate")
        plt.ylim(0, 1.08)
        plt.title("Phase 58 poisoned-witness stress sweep")
        plt.legend()
        plt.tight_layout()
        plt.savefig(out_dir / "phase58_counterexample_forge_metamorphic_tribunal_poison_sweep.png", dpi=150)
        plt.close()
...
def write_examples(out_dir: Path, rows: List[Dict[str, Any]]) -> None:
    if plt is None:
        return
    ex_dir = out_dir / "phase58_examples"
...
def main() -> None:
    ...
    print(f"[{PHASE}] reset continued: from sealed holdout contradiction tribunal to counterexample-forge metamorphic tribunal")
    print(f"[{PHASE}] task: accept the true A->B->C certificate while rejecting false certificates through typed A/B/C role signatures, axis-aware transition probes, contradiction margins, and a counterexample-forge metamorphic tribunal that removes each witness in turn, reselects its quorum from the remaining evidence, replays the branch decision, performs bootstrap and pair cross-examination, then forges fresh counterexample certificates from role swaps, endpoint shortcuts, mirror inversions, temporal scrambles, and cross-family impostors; the proof is rejected if it survives only because the false cases were too easy or too familiar")
    ...
    trials_path = out_dir / "phase58_counterexample_forge_metamorphic_tribunal_trials.csv"
    summary_path = out_dir / "phase58_counterexample_forge_metamorphic_tribunal_summary.json"
    report_path = out_dir / "phase58_counterexample_forge_metamorphic_tribunal_report.md"
    ...
    print(f"[{PHASE}] wrote contradiction-falsifier poison sweep: {out_dir / 'phase58_counterexample_forge_metamorphic_tribunal_poison_sweep.csv'}")
    print(f"[{PHASE}] wrote example png dir: {out_dir / 'phase58_examples'}")
