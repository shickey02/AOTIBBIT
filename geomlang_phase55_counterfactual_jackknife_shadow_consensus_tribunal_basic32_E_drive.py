...
# -*- coding: utf-8 -*-
"""
Phase 55: Counterfactual jackknife-shadow consensus tribunal

Drop-in path:
  E:\BBIT\bbit_geomlang\geomlang_phase55_counterfactual_jackknife_shadow_consensus_tribunal_basic32_E_drive.py

Run:
  python bbit_geomlang/geomlang_phase55_counterfactual_jackknife_shadow_consensus_tribunal_basic32_E_drive.py

Purpose
-------
Phase 51 proved the typed contradiction tribunal could accept the true A -> B -> C certificate,
reject the false certificates, and pass one sealed holdout/null-ablation checks. Phase 55 raises the bar by
adding a jackknife-shadow consensus tribunal: the certificate must keep its selectivity
on two independent panels, including a higher-noise adversarial panel that was never used to
select the winning branch.

Phase 55 adds a jackknife-shadow consensus tribunal on top of the Phase 54 null-ablation near-miss tribunal.
...
Writes into E:\BBIT\outputs_basic32 by default:
  ...
  phase55_jackknife_shadow_consensus_tribunal_trials.csv
  phase55_jackknife_shadow_consensus_tribunal_summary.json
  phase55_jackknife_shadow_consensus_tribunal_report.md
  phase55_jackknife_shadow_consensus_tribunal_poison_sweep.csv
  phase55_examples\*.png
  phase55_jackknife_shadow_consensus_tribunal_accuracy.png
  phase55_jackknife_shadow_consensus_tribunal_rejection_rates.png
  phase55_jackknife_shadow_consensus_tribunal_false_certificate_score_distribution.png
  phase55_jackknife_shadow_consensus_tribunal_true_vs_false_score_margin.png
  phase55_jackknife_shadow_consensus_tribunal_branch_counts.png
  phase55_jackknife_shadow_consensus_tribunal_poison_sweep.png
...
try:
    ...
    plt = None
    print("[55] WARNING: matplotlib unavailable; png plots will be skipped:", exc)


PHASE = "55"
TITLE = "Counterfactual jackknife-shadow consensus tribunal"
PASS_FLAG = "PHASE55_COUNTERFACTUAL_JACKKNIFE_SHADOW_CONSENSUS_TRIBUNAL_PASS"
...
def run_trial(
    ...
    # Phase 55 calibration layer.  Phase 52 proved that false certificates can be
    ...
    # Phase 55: jackknife-shadow consensus tribunal.
    ...
    # lightweight: Phase 55 should not multiply the full certificate grid again; it
    ...
    # is ablated.  Phase 55 asks a harder structural question: does the same
...
def summarize(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # Phase 55 adds a jackknife shadow jury: the certificate must remain good when
    ...
    for fam in FAMILIES:
        ...
        fams[fam] = {
            ...
            "null_ablation_jury_rate": float(np.mean([1.0 if (r["trajectory_correct"] and r["calibrated_false_rejected"] and r["double_sealed_pass"] and r["jackknife_shadow_pass"] and r["near_miss_margin"] > 0.01 and r["relative_false_margin"] > 0.01 and r["log_false_margin"] > 0.01) else 0.0 for r in sub])),
...
def write_report(path: Path, summary: Dict[str, Any]) -> None:
    ...
    lines.append("")
    lines.append(
        "Phase 55 is a falsification phase. Earlier phases proved that the same branch can survive "
...
def plot_outputs(out_dir: Path, rows: List[Dict[str, Any]], summary: Dict[str, Any]) -> None:
    ...
    plt.ylim(0, 1.08)
    plt.ylabel("rate")
    plt.title("Phase 55 final-only vs true-certificate acceptance and false-certificate rejection")
    plt.legend()
    savefig("phase55_jackknife_shadow_consensus_tribunal_accuracy.png")
    ...
    plt.title("Phase 55 closest false certificate type")
    savefig("phase55_jackknife_shadow_consensus_tribunal_rejection_rates.png")
    ...
    plt.ylabel("count")
    plt.title("Phase 55 false certificate score distribution")
    savefig("phase55_jackknife_shadow_consensus_tribunal_false_certificate_score_distribution.png")
    ...
    plt.ylabel("trials")
    plt.title("Phase 55 true-vs-false certificate selectivity margin")
    savefig("phase55_jackknife_shadow_consensus_tribunal_true_vs_false_score_margin.png")
    ...
    plt.ylabel("trials")
    plt.title("Phase 55 calibrated near-miss selectivity margin")
    savefig("phase55_jackknife_shadow_consensus_tribunal_near_miss_margin.png")
    ...
    plt.ylabel("count")
    plt.title("Phase 55 log-scaled false certificate score distribution")
    savefig("phase55_jackknife_shadow_consensus_tribunal_false_certificate_log_score_distribution.png")
    ...
    plt.title("Phase 55 sealed holdout tribunal pass rate by family")
    savefig("phase55_jackknife_shadow_consensus_tribunal_holdout_pass_rate.png")
    ...
    plt.xticks(rotation=25, ha="right")
    plt.ylabel("trials")
    plt.title("Phase 55 predicted branch counts")
    savefig("phase55_jackknife_shadow_consensus_tribunal_branch_counts.png")
...
def poison_sweep(args: argparse.Namespace, out_dir: Path) -> List[Dict[str, Any]]:
    ...
    write_csv(out_dir / "phase55_jackknife_shadow_consensus_tribunal_poison_sweep.csv", sweep_rows)
    ...
    if plt is not None:
        ...
        plt.ylabel("rate")
        plt.ylim(0, 1.08)
        plt.title("Phase 55 poisoned-witness stress sweep")
        plt.legend()
        plt.tight_layout()
        plt.savefig(out_dir / "phase55_jackknife_shadow_consensus_tribunal_poison_sweep.png", dpi=150)
        plt.close()
...
def write_examples(out_dir: Path, rows: List[Dict[str, Any]]) -> None:
    if plt is None:
        return
    ex_dir = out_dir / "phase55_examples"
    ...
    for r in selected:
        ...
        plt.tight_layout()
        plt.savefig(ex_dir / f"phase55_trial_{int(r['trial']):04d}_certificate_scores.png", dpi=140)
        plt.close()
...
def main() -> None:
    ...
    print(f"[{PHASE}] reset continued: from sealed holdout contradiction tribunal to jackknife-shadow consensus tribunal")
    print(f"[{PHASE}] task: accept the true A->B->C certificate while rejecting false certificates through typed A/B/C role signatures, axis-aware transition probes, contradiction margins, and a jackknife-shadow consensus tribunal that removes each witness in turn, reselects its quorum from the remaining evidence, replays the branch decision, tests scale-normalized false margins, hard negative pressure, adversarial near-miss separation, and refuses proofs that only work because one witness or one ordering dominates")
    ...
    trials_path = out_dir / "phase55_jackknife_shadow_consensus_tribunal_trials.csv"
    summary_path = out_dir / "phase55_jackknife_shadow_consensus_tribunal_summary.json"
    report_path = out_dir / "phase55_jackknife_shadow_consensus_tribunal_report.md"
    ...
    print(f"[{PHASE}] wrote contradiction-falsifier poison sweep: {out_dir / 'phase55_jackknife_shadow_consensus_tribunal_poison_sweep.csv'}")
    print(f"[{PHASE}] wrote example png dir: {out_dir / 'phase55_examples'}")
