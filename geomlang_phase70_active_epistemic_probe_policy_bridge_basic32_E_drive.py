"""
Phase 70 — Active epistemic probe policy bridge for Basic32 raster grammar.

Drop this file into:
    E:\BBIT\bbit_geomlang\geomlang_phase70_active_epistemic_probe_policy_bridge_basic32_E_drive.py

Run:
    python bbit_geomlang/geomlang_phase70_active_epistemic_probe_policy_bridge_basic32_E_drive.py

Phase 70 continues the Phase 69 epistemic reset.  Phase 69 learned not to
hallucinate when no-luma rasters are underdetermined.  Phase 70 adds the next
step: an active perception policy.  When the no-luma observation is not enough,
the system requests a minimal luma probe and then re-solves the scene grammar.
...
try:
    ...
    raise RuntimeError(
        "Phase 70 expects Phase 68 and Phase 69 to be present beside it in bbit_geomlang. "
...
def needs_probe(no_luma_obs: Dict[str, object], no_luma_pred: Dict[str, object]) -> Tuple[bool, str]:
    """Probe policy using only current-image epistemic status and confidence."""
    if bool(no_luma_obs.get("abstain", False)):
        return True, str(no_luma_obs.get("reason", "abstain"))
    if safe_float(no_luma_pred.get("margin", 0.0)) < LOW_CONF_MARGIN:
        return True, "low_confidence_observable"
    return False, "observable_confident"
...
def main() -> None:
    ...
    print(f"[{PHASE}] reset continued: from epistemic observability to active perception")
    print(f"[{PHASE}] task: probe only when the no-luma raster is underdetermined or low-confidence, then recover scene grammar")
    ...
    for scene_name, shape_a, shape_b, relation in SCENES:
        for k in range(TRIALS_PER_SCENE):
            # Start from the hard no-luma observation.  This keeps Phase 70 honest:
            ...
            policy_correct = int((result["probe_requested"] == 1 and result["probe_reason"] != "observable_confident") or (result["probe_requested"] == 0 and result["probe_reason"] == "observable_confident"))
            ...
            if example_count < 40 and k % 12 == 0:
                save_img(EXAMPLE_DIR / f"phase70_noluma_{example_count:03d}_{scene_name}.png", img_noluma)
                if result["probe_requested"] == 1:
                    probe_img = active_probe_image(shape_a, shape_b, relation, rng)
                    save_img(EXAMPLE_DIR / f"phase70_probe_{example_count:03d}_{scene_name}.png", probe_img)
                example_count += 1
    ...
    passed = (
        ...
        scene_acc >= blind_scene_acc + 0.20
    )
    ...
    # Write data outputs.
    trials_path = OUT / "phase70_active_epistemic_probe_policy_bridge_trials.csv"
    ...
    summary = {
        ...
        "notes": [
            "Phase 70 treats abstention as an active perception trigger, not as failure.",
            "The active probe is a minimal luma/contrast observation used only when no-luma evidence is underdetermined or low confidence.",
            "Success requires active accuracy to beat the blind no-luma forced-answer baseline by a large margin.",
        ],
    }
    summary_path = OUT / "phase70_active_epistemic_probe_policy_bridge_summary.json"
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2)

    report_path = OUT / "phase70_active_epistemic_probe_policy_bridge_report.md"
    with open(report_path, "w", encoding="utf-8") as f:
        f.write(f"# Phase {PHASE}: {TITLE}\n\n")
        f.write("## Summary\n\n")
        for k in [
            "pass", "trials", "scene_accuracy", "relation_accuracy", "concept_pair_accuracy",
            "probe_rate", "policy_accuracy", "no_hallucination_accuracy",
        ...
        f.write(
            ...
            "Phase 70 converts that uncertainty into action.  The policy first tries the no-luma observation.  If the image is underdetermined or too low-confidence, it requests a targeted luma probe and then resolves the grammar.\n"
        )

    # Visual outputs.
    bar_plot(
        OUT / "phase70_active_epistemic_probe_policy_metrics.png",
        "Phase 70 active epistemic probe metrics",
    ...
    bar_plot(
        OUT / "phase70_active_epistemic_probe_rate_by_scene.png",
        "Phase 70 probe rate by scene",
        [s["scene"] for s in summaries],
        [s["probe_rate"] for s in summaries],
        ylabel="probe rate",
    )
    bar_plot(
        OUT / "phase70_active_epistemic_gain_by_scene.png",
        "Phase 70 active probe gain over blind no-luma",
        [s["scene"] for s in summaries],
    ...
    histogram_plot(
        OUT / "phase70_active_epistemic_margin_distribution.png",
        "Phase 70 active epistemic winner margin distribution",
        [float(r["margin"]) for r in rows],
        "runner-up semantic score - winner score",
    )
    confusion_plot(
        OUT / "phase70_active_epistemic_scene_confusion.png",
        "Phase 70 active scene confusion",
        scene_labels,
        scene_conf,
    )
    confusion_plot(
        OUT / "phase70_active_epistemic_relation_confusion.png",
        "Phase 70 active relation confusion",
        RELATIONS,
        relation_conf,
    )

    print(f"[{PHASE}] PHASE70_ACTIVE_EPISTEMIC_PROBE_POLICY_BRIDGE_PASS={passed}")
    print(
        f"[{PHASE}] scene_accuracy={scene_acc:.4f} relation_accuracy={relation_acc:.4f} "
        f"concept_pair_accuracy={pair_acc:.4f} probe_rate={probe_rate:.4f} "
