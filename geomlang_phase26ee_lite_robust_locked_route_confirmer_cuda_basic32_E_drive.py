...
# -*- coding: utf-8 -*-
"""
Phase 26EE-LITE: Robust locked-route confirmer / verifier for Basic32 geomlang.
...
PHASE = "26EE-LITE"
TITLE = "Robust locked-route confirmer and strict-stability verifier"
...
def summarize_ee(route_results: pd.DataFrame, target_cap: float, target_tan: float, relaxed_tan: float, margin_tan: float, margin_cap: float) -> pd.DataFrame:
    ...
    for rn, g in route_results.groupby("route_name", dropna=False):
        ...
        rows.append({
            ...
            "ee_robust_score": ec_score,
        })
    out = pd.DataFrame(rows)
    if out.empty:
        return out
    return out.sort_values(
        ["stable_strict_all_seeds", "strict_margin_all_seeds", "ee_robust_score", "explicit_label_count"],
...
def plot_capture_tangent(df: pd.DataFrame, out_path: Path, target_cap: float, target_tan: float, relaxed_tan: float) -> None:
    ...
    c = df.get("ee_robust_score", df.get("dz_route_score_run", pd.Series(np.zeros(len(df))))).astype(float)
    ...
    plt.title("26EE-LITE compressed/locked routes: capture vs tangent")
    plt.colorbar(label="EE run score")
    plt.legend()
    plt.tight_layout()
...
def plot_route_scores(summary: pd.DataFrame, out_path: Path, title: str = "26EE-LITE route margin scores") -> None:
    if summary.empty:
        return
    top = summary.head(18).iloc[::-1]
    plt.figure(figsize=(14, max(6, 0.5 * len(top))))
    plt.barh(top["route_name"], top["ee_robust_score"])
    for i, r in enumerate(top.itertuples()):
        plt.text(float(r.ee_robust_score), i, f" cap={r.min_worst_capture_rate:.3f} tan={r.max_worst_tangent_ratio:.3f} labels={int(r.explicit_label_count)}", va="center", fontsize=8)
    plt.xlabel("EE robustness score")
    plt.title(title)
    plt.tight_layout()
    plt.savefig(out_path, dpi=140)
    plt.close()
...
def plot_pass_rates(summary: pd.DataFrame, out_path: Path) -> None:
    ...
    plt.title("26EE-LITE locked/compressed route pass rates")
    plt.legend()
    plt.tight_layout()
    plt.savefig(out_path, dpi=140)
    plt.close()
...
def plot_label_stress(cases: pd.DataFrame, out_path: Path) -> None:
    ...
    plt.title("26EE-LITE remaining label stress inside selected route")
    plt.tight_layout()
    plt.savefig(out_path, dpi=140)
    plt.close()
...
def run(args: argparse.Namespace) -> None:
    ...
    route_summary = summarize_ee(route_results, args.target_capture, args.target_tangent, args.relaxed_tangent, args.margin_tangent, args.margin_capture)
    if not route_summary.empty:
        score_lookup = route_summary.set_index("route_name")["ee_robust_score"].to_dict()
        route_results["ee_robust_score"] = route_results["route_name"].map(score_lookup).fillna(route_results.get("dz_route_score_run", 0.0))
    ...
    # Output artifacts.
    pool_cases.to_csv(ROOT / "phase26ee_lite_pool_case_results.csv", index=False)
    route_cases.to_csv(ROOT / "phase26ee_lite_route_cases.csv", index=False)
    route_results.to_csv(ROOT / "phase26ee_lite_route_results.csv", index=False)
    route_summary.to_csv(ROOT / "phase26ee_lite_route_summary.csv", index=False)
    if best_route_map:
        (ROOT / "phase26ee_lite_locked_route_map.json").write_text(json.dumps(best_route_map, indent=2, sort_keys=True), encoding="utf-8")

    plot_route_scores(route_summary, ROOT / "phase26ee_lite_route_scores.png")
    plot_pass_rates(route_summary, ROOT / "phase26ee_lite_pass_rates.png")
    plot_capture_tangent(route_results, ROOT / "phase26ee_lite_route_capture_vs_tangent.png", args.target_capture, args.target_tangent, args.relaxed_tangent)
    plot_label_stress(best_cases, ROOT / "phase26ee_lite_best_route_label_stress.png")
    ...
    summary = {
        ...
        "best_ee_robust_score": None if not len(best) else fnum(best.get("ee_robust_score")),
        ...
        "outputs": [
            ...
            "phase26ee_lite_route_cases.csv",
            "phase26ee_lite_route_results.csv",
            "phase26ee_lite_route_summary.csv",
            "phase26ee_lite_locked_route_map.json",
            "phase26ee_lite_summary.json",
            "phase26ee_lite_route_scores.png",
            "phase26ee_lite_pass_rates.png",
            "phase26ee_lite_route_capture_vs_tangent.png",
            "phase26ee_lite_best_route_label_stress.png",
        ],
    }
    (ROOT / "phase26ee_lite_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--verify-seed-count", type=int, default=10)
    ap.add_argument("--audit-repeats", type=int, default=5)
    ap.add_argument("--pool-size", type=int, default=24)
    ap.add_argument("--extra-jitters", type=int, default=12)
    ...
    ap.add_argument("--target-q20", type=float, default=-4.0)
    ...
    ap.add_argument("--max-swap-labels", type=int, default=7)
    ap.add_argument("--max-swaps-per-label", type=int, default=3)
    ap.add_argument("--max-swap-routes", type=int, default=24)
    return ap
