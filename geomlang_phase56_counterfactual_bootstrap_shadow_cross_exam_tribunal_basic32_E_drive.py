...
# -*- coding: utf-8 -*-
r"""
Phase 56: Counterfactual bootstrap-shadow cross-examination tribunal

Drop-in path:
  E:\BBIT\bbit_geomlang\geomlang_phase56_counterfactual_bootstrap_shadow_cross_exam_tribunal_basic32_E_drive.py

Run:
  python bbit_geomlang/geomlang_phase56_counterfactual_bootstrap_shadow_cross_exam_tribunal_basic32_E_drive.py

Purpose
-------
Phase 51 proved the typed contradiction tribunal could accept the true A -> B -> C certificate,
reject the false certificates, and pass one sealed holdout/null-ablation checks. Phase 56 raises the bar by
adding a bootstrap-shadow cross-examination tribunal: the certificate must keep its selectivity
on two independent panels, including a higher-noise adversarial panel that was never used to
select the winning branch.

Phase 56 adds a bootstrap-shadow cross-examination tribunal on top of the Phase 54 null-ablation near-miss tribunal.
...
Writes into E:\BBIT\outputs_basic32 by default:
  ...
  phase56_bootstrap_shadow_cross_exam_tribunal_trials.csv
  phase56_bootstrap_shadow_cross_exam_tribunal_summary.json
  phase56_bootstrap_shadow_cross_exam_tribunal_report.md
  phase56_bootstrap_shadow_cross_exam_tribunal_poison_sweep.csv
  phase56_examples\*.png
  phase56_bootstrap_shadow_cross_exam_tribunal_accuracy.png
  phase56_bootstrap_shadow_cross_exam_tribunal_rejection_rates.png
  phase56_bootstrap_shadow_cross_exam_tribunal_false_certificate_score_distribution.png
  phase56_bootstrap_shadow_cross_exam_tribunal_true_vs_false_score_margin.png
  phase56_bootstrap_shadow_cross_exam_tribunal_branch_counts.png
  phase56_bootstrap_shadow_cross_exam_tribunal_poison_sweep.png
...
try:
    ...
    plt = None
    print("[56] WARNING: matplotlib unavailable; png plots will be skipped:", exc)


PHASE = "56"
TITLE = "Counterfactual bootstrap-shadow cross-examination tribunal"
PASS_FLAG = "PHASE56_COUNTERFACTUAL_BOOTSTRAP_SHADOW_CROSS_EXAM_TRIBUNAL_PASS"
...
def run_trial(
    ...
    # Phase 56 calibration layer.  Phase 52 proved that false certificates can be
    ...
    # Phase 56: bootstrap-shadow cross-examination tribunal.
    ...
    # lightweight: Phase 56 should not multiply the full certificate grid again; it
    ...
    # is ablated.  Phase 56 asks a harder structural question: does the same
...
def summarize(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # Phase 56 adds a jackknife/bootstrap shadow jury: the certificate must remain good when
    ...
    for fam in FAMILIES:
        ...
        fams[fam] = {
            ...
            "null_ablation_jury_rate": float(np.mean([1.0 if (r["trajectory_correct"] and r["calibrated_false_rejected"] and r["double_sealed_pass"] and r["jackknife_shadow_pass"] and r["cross_exam_pass"] and r["near_miss_margin"] > 0.01 and r["relative_false_margin"] > 0.01 and r["log_false_margin"] > 0.01) else 0.0 for r in sub])),
...
def write_report(path: Path, summary: Dict[str, Any]) -> None:
    ...
    lines.append("")
    lines.append(
        "Phase 56 is a falsification phase. Earlier phases proved that the same branch can survive "
...
def plot_outputs(out_dir: Path, rows: List[Dict[str, Any]], summary: Dict[str, Any]) -> None:
    ...
    plt.ylim(0, 1.08)
    plt.ylabel("rate")
    plt.title("Phase 56 final-only vs true-certificate acceptance and false-certificate rejection")
    plt.legend()
    savefig("phase56_bootstrap_shadow_cross_exam_tribunal_accuracy.png")
    ...
    plt.title("Phase 56 closest false certificate type")
    savefig("phase56_bootstrap_shadow_cross_exam_tribunal_rejection_rates.png")
    ...
    plt.ylabel("count")
    plt.title("Phase 56 false certificate score distribution")
    savefig("phase56_bootstrap_shadow_cross_exam_tribunal_false_certificate_score_distribution.png")
    ...
    plt.ylabel("trials")
    plt.title("Phase 56 true-vs-false certificate selectivity margin")
    savefig("phase56_bootstrap_shadow_cross_exam_tribunal_true_vs_false_score_margin.png")
    ...
    plt.ylabel("trials")
    plt.title("Phase 56 calibrated near-miss selectivity margin")
    savefig("phase56_bootstrap_shadow_cross_exam_tribunal_near_miss_margin.png")
    ...
    plt.ylabel("count")
    plt.title("Phase 56 log-scaled false certificate score distribution")
    savefig("phase56_bootstrap_shadow_cross_exam_tribunal_false_certificate_log_score_distribution.png")
    ...
    plt.title("Phase 56 sealed holdout tribunal pass rate by family")
    savefig("phase56_bootstrap_shadow_cross_exam_tribunal_holdout_pass_rate.png")
    ...
    plt.xticks(rotation=25, ha="right")
    plt.ylabel("trials")
    plt.title("Phase 56 predicted branch counts")
    savefig("phase56_bootstrap_shadow_cross_exam_tribunal_branch_counts.png")

    # Phase 56 cross-exam rates: jackknife, bootstrap, and pair panels.
    ...
    plt.ylim(0, 1.08)
    plt.ylabel("rate")
    plt.title("Phase 56 jackknife/bootstrap/pair cross-exam consensus")
    plt.legend()
    savefig("phase56_bootstrap_shadow_cross_exam_tribunal_cross_exam_rates.png")
...
def poison_sweep(args: argparse.Namespace, out_dir: Path) -> List[Dict[str, Any]]:
    ...
    write_csv(out_dir / "phase56_bootstrap_shadow_cross_exam_tribunal_poison_sweep.csv", sweep_rows)
    ...
    if plt is not None:
        ...
        plt.ylabel("rate")
        plt.ylim(0, 1.08)
        plt.title("Phase 56 poisoned-witness stress sweep")
        plt.legend()
        plt.tight_layout()
        plt.savefig(out_dir / "phase56_bootstrap_shadow_cross_exam_tribunal_poison_sweep.png", dpi=150)
        plt.close()
...
def write_examples(out_dir: Path, rows: List[Dict[str, Any]]) -> None:
    if plt is None:
        return
    ex_dir = out_dir / "phase56_examples"
...
def main() -> None:
    ...
    print(f"[{PHASE}] reset continued: from sealed holdout contradiction tribunal to bootstrap-shadow cross-examination tribunal")
    print(f"[{PHASE}] task: accept the true A->B->C certificate while rejecting false certificates through typed A/B/C role signatures, axis-aware transition probes, contradiction margins, and a bootstrap-shadow cross-examination tribunal that removes each witness in turn, reselects its quorum from the remaining evidence, replays the branch decision, tests scale-normalized false margins, hard negative pressure, adversarial near-miss separation, and refuses proofs that only work because one witness or one ordering dominates")
    ...
    trials_path = out_dir / "phase56_bootstrap_shadow_cross_exam_tribunal_trials.csv"
    summary_path = out_dir / "phase56_bootstrap_shadow_cross_exam_tribunal_summary.json"
    report_path = out_dir / "phase56_bootstrap_shadow_cross_exam_tribunal_report.md"
    ...
    print(f"[{PHASE}] wrote contradiction-falsifier poison sweep: {out_dir / 'phase56_bootstrap_shadow_cross_exam_tribunal_poison_sweep.csv'}")
    print(f"[{PHASE}] wrote example png dir: {out_dir / 'phase56_examples'}")
