#!/usr/bin/env python3
# AOTI LEVEL 44: THE BANKROLL MANAGER (FINAL FIX)
# - New DB file 'aoti_live.db' to fix schema crash.
# - Includes Position Sizing (% of Equity).
# - Includes "Honest Wallet" tracking.
...
# CHANGED: New filename to force fresh DB creation
DB_FILE = 'aoti_live.db'
...
class TradeLog(Base):
    ...
    # This is the new column that caused the crash
    ...
    def manage(self, executor):
        ...
        for trade in holdings:
            ...
            if slope < sell_threshold:
                ...
                # Handle legacy trades with no qty
                qty_to_sell = trade.qty if trade.qty else (trade.price / trade.price)
