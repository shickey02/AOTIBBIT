#!/usr/bin/env python3
# geomlang_lie_group_latent256.py
#
# Lie-group style analysis of relations in the edges+relscale model
# with LATENT_DIM = 256 (same architecture as 64x64 edges model).
#
# IMPORTANT: The checkpoint scene_model_edges_relscale256.pt was trained
# with 64x64 images, not 256x256. So we keep IMG_SIZE=64 here.
...
# Config
# -----------------------
IMG_SIZE   = 64          # must match training architecture
NUM_CH     = 3           # red, blue, edges
LATENT_DIM = 256         # this is the "256" we care about
...
# -----------------------
# Scene generation (64x64) – same as edges64 arithmetic script
# -----------------------
...
# (this matches the checkpoint shapes)
# -----------------------
...
def load_scene_model():
    print(f"[lie256] Loading SceneModel from {CKPT_SCENEMODEL}")
...
def pca_on_cloud(v, name, max_components=32):
    ...
    print(f"\n[lie256] PCA on {name} difference cloud:")
...
def main():
    print(f"[lie256] Using device: {DEVICE}")
    model = load_scene_model()

    print(f"[lie256] Generating dataset: N={N_SAMPLES}")
    ...
    for r in range(5):
        n = int((rel_np == r).sum())
        print(f"[lie256] relation {r}: {REL_NAMES[r]:>7}, count={n}")
    ...
    k = min(rank_lr, rank_ab, 16)  # compare top-k directions
    U_lr = pca_lr.components_[:k].T
    U_ab = pca_ab.components_[:k].T

    angles = principal_angles(U_lr, U_ab)
    print("\n[lie256] Principal angles between LR and AB subspaces (deg):")
    for i, a in enumerate(angles, 1):
        print(f"  θ{i:02d} = {a * 180.0 / math.pi:6.2f}°")

    # simple PCA scatter + scree plots
    ...
    plt.tight_layout()
    out_scatter = os.path.join(OUT_DIR, "lie_group_diffcloud_pca_latent256.png")
    plt.savefig(out_scatter)
    print(f"[lie256] Saved diff-cloud PCA scatter -> {out_scatter}")
    ...
    plt.tight_layout()
    out_scree = os.path.join(OUT_DIR, "lie_group_scree_latent256.png")
    plt.savefig(out_scree)
    print(f"[lie256] Saved scree plots -> {out_scree}")
