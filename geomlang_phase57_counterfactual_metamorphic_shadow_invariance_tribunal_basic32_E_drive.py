...
# -*- coding: utf-8 -*-
r"""
Phase 57: Counterfactual metamorphic-shadow invariance tribunal

Drop-in path:
  E:\BBIT\bbit_geomlang\geomlang_phase57_counterfactual_metamorphic_shadow_invariance_tribunal_basic32_E_drive.py

Run:
  python bbit_geomlang/geomlang_phase57_counterfactual_metamorphic_shadow_invariance_tribunal_basic32_E_drive.py

Purpose
-------
Phase 51 proved the typed contradiction tribunal could accept the true A -> B -> C certificate,
reject the false certificates, and pass one sealed holdout/null-ablation checks. Phase 57 raises the bar by
...
Phase 57 adds a metamorphic-shadow invariance tribunal on top of the Phase 54 null-ablation near-miss tribunal.
...
Writes into E:\BBIT\outputs_basic32 by default:
  ...
  phase57_metamorphic_shadow_invariance_tribunal_trials.csv
  phase57_metamorphic_shadow_invariance_tribunal_summary.json
  phase57_metamorphic_shadow_invariance_tribunal_report.md
  phase57_metamorphic_shadow_invariance_tribunal_poison_sweep.csv
  phase57_examples\*.png
  phase57_metamorphic_shadow_invariance_tribunal_accuracy.png
  phase57_metamorphic_shadow_invariance_tribunal_rejection_rates.png
  phase57_metamorphic_shadow_invariance_tribunal_false_certificate_score_distribution.png
  phase57_metamorphic_shadow_invariance_tribunal_true_vs_false_score_margin.png
  phase57_metamorphic_shadow_invariance_tribunal_branch_counts.png
  phase57_metamorphic_shadow_invariance_tribunal_poison_sweep.png
...
try:
    ...
    plt = None
    print("[57] WARNING: matplotlib unavailable; png plots will be skipped:", exc)


PHASE = "57"
TITLE = "Counterfactual metamorphic-shadow invariance tribunal"
PASS_FLAG = "PHASE57_COUNTERFACTUAL_METAMORPHIC_SHADOW_INVARIANCE_TRIBUNAL_PASS"
...
def run_trial(
    ...
    # Phase 57 calibration layer.  Phase 52 proved that false certificates can be
    ...
    # Phase 57: metamorphic-shadow invariance tribunal.
    ...
    # lightweight: Phase 57 should not multiply the full certificate grid again; it
    ...
    # is ablated.  Phase 57 asks a harder structural question: does the same
...
def summarize(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # Phase 57 adds a jackknife/bootstrap shadow jury: the certificate must remain good when
    ...
    return {
        ...
        "metamorphic_role_swap_veto_rate": metamorphic_role_swap_veto_rate,
        "metamorphic_score_scale_veto_rate": metamorphic_score_scale_veto_rate,
        "metamorphic_consensus_shadow_rate": metamorphic_consensus_shadow_rate,
...
def write_report(path: Path, summary: Dict[str, Any]) -> None:
    ...
    lines.append("")
    lines.append(
        "Phase 57 is a falsification phase. Earlier phases proved that the same branch can survive "
...
def plot_outputs(out_dir: Path, rows: List[Dict[str, Any]], summary: Dict[str, Any]) -> None:
    ...
    plt.ylim(0, 1.08)
    plt.ylabel("rate")
    plt.title("Phase 57 final-only vs true-certificate acceptance and false-certificate rejection")
    plt.legend()
    savefig("phase57_metamorphic_shadow_invariance_tribunal_accuracy.png")
    ...
    plt.title("Phase 57 closest false certificate type")
    savefig("phase57_metamorphic_shadow_invariance_tribunal_rejection_rates.png")
    ...
    plt.ylabel("count")
    plt.title("Phase 57 false certificate score distribution")
    savefig("phase57_metamorphic_shadow_invariance_tribunal_false_certificate_score_distribution.png")
    ...
    plt.ylabel("trials")
    plt.title("Phase 57 true-vs-false certificate selectivity margin")
    savefig("phase57_metamorphic_shadow_invariance_tribunal_true_vs_false_score_margin.png")
    ...
    plt.ylabel("trials")
    plt.title("Phase 57 calibrated near-miss selectivity margin")
    savefig("phase57_metamorphic_shadow_invariance_tribunal_near_miss_margin.png")
    ...
    plt.ylabel("count")
    plt.title("Phase 57 log-scaled false certificate score distribution")
    savefig("phase57_metamorphic_shadow_invariance_tribunal_false_certificate_log_score_distribution.png")
    ...
    plt.title("Phase 57 sealed holdout tribunal pass rate by family")
    savefig("phase57_metamorphic_shadow_invariance_tribunal_holdout_pass_rate.png")
    ...
    plt.xticks(rotation=25, ha="right")
    plt.ylabel("trials")
    plt.title("Phase 57 predicted branch counts")
    savefig("phase57_metamorphic_shadow_invariance_tribunal_branch_counts.png")

    # Phase 57 cross-exam rates: jackknife, bootstrap, and pair panels.
    ...
    plt.ylim(0, 1.08)
    plt.ylabel("rate")
    plt.title("Phase 57 jackknife/bootstrap/pair cross-exam consensus")
    plt.legend()
    savefig("phase57_metamorphic_shadow_invariance_tribunal_cross_exam_rates.png")
...
def poison_sweep(args: argparse.Namespace, out_dir: Path) -> List[Dict[str, Any]]:
    ...
    write_csv(out_dir / "phase57_metamorphic_shadow_invariance_tribunal_poison_sweep.csv", sweep_rows)
    ...
    if plt is not None:
        ...
        plt.ylabel("rate")
        plt.ylim(0, 1.08)
        plt.title("Phase 57 poisoned-witness stress sweep")
        plt.legend()
        plt.tight_layout()
        plt.savefig(out_dir / "phase57_metamorphic_shadow_invariance_tribunal_poison_sweep.png", dpi=150)
        plt.close()
...
def write_examples(out_dir: Path, rows: List[Dict[str, Any]]) -> None:
    if plt is None:
        return
    ex_dir = out_dir / "phase57_examples"
...
def main() -> None:
    ...
    print(f"[{PHASE}] reset continued: from sealed holdout contradiction tribunal to metamorphic-shadow invariance tribunal")
    print(f"[{PHASE}] task: accept the true A->B->C certificate while rejecting false certificates through typed A/B/C role signatures, axis-aware transition probes, contradiction margins, and a metamorphic-shadow invariance tribunal that removes each witness in turn, reselects its quorum from the remaining evidence, replays the branch decision, performs bootstrap and pair cross-examination, then runs a final metamorphic invariance veto over role-swaps, scale-normalized near-misses, consensus shadows, and false-certificate pressure so the proof is rejected if it only survives under one accidental presentation of the evidence")
    ...
    trials_path = out_dir / "phase57_metamorphic_shadow_invariance_tribunal_trials.csv"
    summary_path = out_dir / "phase57_metamorphic_shadow_invariance_tribunal_summary.json"
    report_path = out_dir / "phase57_metamorphic_shadow_invariance_tribunal_report.md"
    ...
    print(f"[{PHASE}] wrote contradiction-falsifier poison sweep: {out_dir / 'phase57_metamorphic_shadow_invariance_tribunal_poison_sweep.csv'}")
    print(f"[{PHASE}] wrote example png dir: {out_dir / 'phase57_examples'}")
