#!/usr/bin/env python3
# DYNAMIC YAHTZEE v2: THE BONUS HUNTER
# Adds awareness of the 63-point Upper Section Bonus to the Dynamic Agent.
...
class YahtzeeGame:
    def __init__(self):
        ...
        self.turns_left = 13
        self.total_score = 0
        self.upper_score = 0 # Track progress toward 63
    ...
    def check_bonus(self):
        if self.upper_score >= 63: return 35
        return 0
...
class BonusHunterAgent:
    def __init__(self, brain, centroids, genome=None):
        ...
        # 0-8: Standard Dynamic
        # 9: BONUS OBSESSION (New) - Sensitivity to Upper Section progress
        
        if genome is None:
            # Start with successful Dynamic genome values
            self.genome = np.array([
                ...
                1.5,  # T->Risk
                1.0,  # D->Greed
                2.0,  # D->Risk (Adrenaline!)
                2.0,  # Bonus Weight
                3.0   # Bonus Obsession (Start HIGH)
            ])
            self.genome += np.random.normal(0, 0.2, 10)
        else:
            self.genome = genome
    ...
    def modulate_personality(self, game):
        ...
        # Bonus Deficit!
        # By turn X, we should have roughly X/6 * 63 points in upper section
        expected_upper = (turns_passed / 13.0) * 63
        upper_deficit = max(0, expected_upper - game.upper_score)
        ...
        # Obsession scales with Upper Deficit
        curr_obsession = self.genome[9] * (1.0 + (upper_deficit / 10.0))
    ...
    def evaluate_target(self, dice, category, game, personality):
        ...
        if category <= 6:
            ...
            # If we have 3 or more, this is CRITICAL to lock in
            if count >= 3:
                utility += personality['obsession'] * 20
                
            # If we have 4 or more, almost Infinite Utility
            if count >= 4:
                utility += personality['obsession'] * 50
    ...
    def play_turn(self, game):
        ...
        # SMART DUMP LOGIC (The Safety Net)
        if score == 0:
            open_cats = game.get_open_slots()
            # 1. Can we score elsewhere?
            for cat in open_cats:
                if game.calculate_score(dice, cat) > 0:
                    best_cat = cat; score = game.calculate_score(dice, cat); break
            
            # 2. If not, minimize pain.
            if score == 0:
                # Dump Priority: Ones > Twos > Yahtzee (if risky) > Chance
                dump_order = [1, 2, 3, 12, 4, 5, 6] # Ones/Twos are cheap dumps
                found_dump = False
                for d_cat in dump_order:
                    if d_cat in open_cats:
                        best_cat = d_cat
                        found_dump = True
                        break
                if not found_dump:
                    best_cat = open_cats[0] # Whatever is left
                
        game.commit_score(best_cat, score)

def run_sim(data_dir):
    print("--- DYNAMIC v2: BONUS HUNTER ---")
    ...
    pop_size = 60
    population = [BonusHunterAgent(brain, centroids) for _ in range(pop_size)]
    
    goal = 180 # We expect better performance immediately
    ...
    for gen in range(1, 101):
        ...
        for agent in population:
            ...
            # Add Bonus to final score for evolution tracking
        ...
        best = parents[0]
        print(f"   Top: Obsession={best.genome[9]:.2f} Risk={best.genome[2]:.2f}")
        
        new_pop = []
        while len(new_pop) < pop_size:
            p = random.choice(parents)
            child_genome = np.array(p.genome) + np.random.normal(0, 0.15, 10)
            new_pop.append(BonusHunterAgent(brain, centroids, list(child_genome)))
