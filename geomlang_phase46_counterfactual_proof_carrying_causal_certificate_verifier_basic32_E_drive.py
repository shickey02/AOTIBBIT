...
# -*- coding: utf-8 -*-
r"""
Phase 46: Counterfactual proof-carrying causal certificate verifier
...
Phase 46 adds a proof-carrying certificate problem: the system now receives several source
...
Task:
  ...
  Phase 46 adds a proof-carrying causal certificate check: the consensus is not merely recomputed: the answer is treated as if it must
  carry a compact causal certificate. The certificate consists of the full
...
Outputs:
  E:\BBIT\outputs_basic32\phase46_causal_certificate_verifier_trials.csv
  E:\BBIT\outputs_basic32\phase46_causal_certificate_verifier_summary.json
  E:\BBIT\outputs_basic32\phase46_causal_certificate_verifier_report.md
  E:\BBIT\outputs_basic32\phase46_causal_certificate_poison_sweep.csv
  E:\BBIT\outputs_basic32\phase46_examples\*.png
...
PHASE = "46"
TITLE = "Counterfactual proof-carrying causal certificate verifier"
SCRIPT_NAME = "geomlang_phase46_counterfactual_proof_carrying_causal_certificate_verifier_basic32_E_drive.py"
ROOT = Path(r"E:\BBIT") if Path(r"E:\BBIT").exists() else Path.cwd()
OUT = ROOT / "outputs_basic32"
EXAMPLE_DIR = OUT / "phase46_examples"
...
def infer_consensus(witnesses: List[Tuple[np.ndarray, np.ndarray, np.ndarray, bool]]) -> Tuple[Affine, Affine, Dict[str, float]]:
    ...
    # Phase 46 certificate component 1: leave-one replay.
    ...
    # Phase 46 causal-certificate sufficiency diagnostic: rebuild the weighted-median
    ...
    # Phase 46 causal-certificate diagnostic: now ask a harder question.
    ...
    return c1, c2, {
        ...
        "causal_certificate_error": float(np.mean(coalition_drifts)),
        "causal_certificate_max_error": float(np.max(coalition_drifts)),
        "causal_certificate_best_error": float(np.min(coalition_drifts)),
        "causal_certificate_size": float(coalition_size),
        "causal_certificate_mean_fit": float(np.mean(coalition_scores)),
...
def aggregate(rows: List[Dict[str, object]]) -> Dict[str, object]:
    ...
    summary: Dict[str, object] = {
        ...
        "mean_causal_certificate_error": float(arr("causal_certificate_error").mean()),
        "mean_causal_certificate_max_error": float(arr("causal_certificate_max_error").mean()),
        "mean_causal_certificate_best_error": float(arr("causal_certificate_best_error").mean()),
        "mean_causal_certificate_size": float(arr("causal_certificate_size").mean()),
        "mean_causal_certificate_fit": float(arr("causal_certificate_mean_fit").mean()),
    ...
    for fam in FAMILIES:
        ...
        fams[fam] = {
            ...
            "mean_causal_certificate_error": float(aa("causal_certificate_error").mean()),
...
def plot_accuracy(summary: Dict[str, object]) -> None:
    ...
    ax.set_title("Phase 46 counterfactual proof-carrying causal certificate reasoning: accuracy by hidden family")
    for i, v in enumerate(vals):
        ax.text(v + 0.01, i, f"{v:.2f}", va="center")
    fig.tight_layout()
    fig.savefig(OUT / "phase46_causal_certificate_verifier_accuracy.png", dpi=140)
    plt.close(fig)
...
def plot_baseline_gap(summary: Dict[str, object]) -> None:
    ...
    ax.bar(x + width/2, traj, width, label="causal-certificate verifier")
    ax.set_ylim(0, 1.05)
    ax.set_ylabel("accuracy")
    ax.set_title("Phase 46 final-only baseline vs proof-carrying causal certificate")
    ax.set_xticks(x)
    ax.set_xticklabels(labels, rotation=22, ha="right")
    ax.legend()
    fig.tight_layout()
    fig.savefig(OUT / "phase46_causal_certificate_verifier_baseline_gap.png", dpi=140)
    plt.close(fig)
...
def plot_branch_counts(rows: List[Dict[str, object]]) -> None:
    ...
    ax.bar(labels, vals)
    ax.set_ylabel("trials")
    ax.set_title("Phase 46 predicted branch counts")
    ax.tick_params(axis="x", rotation=25)
    fig.tight_layout()
    fig.savefig(OUT / "phase46_causal_certificate_verifier_branch_counts.png", dpi=140)
    plt.close(fig)
...
def write_poison_sweep(rows: List[Dict[str, float]]) -> None:
    path = OUT / "phase46_causal_certificate_poison_sweep.csv"
    with path.open("w", newline="", encoding="utf-8") as f:
    ...
    ax.set_ylabel("accuracy")
    ax.set_title("Phase 46 poisoned-witness stress sweep")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(OUT / "phase46_causal_certificate_verifier_poison_sweep.png", dpi=140)
    plt.close(fig)
...
def write_report(summary: Dict[str, object], path: Path) -> None:
    ...
    lines.append(f"- mean causal-certificate error: `{summary['mean_causal_certificate_error']:.6f}`")
    lines.append(f"- mean causal-certificate max error: `{summary['mean_causal_certificate_max_error']:.6f}`")
    lines.append(f"- mean causal-certificate best error: `{summary['mean_causal_certificate_best_error']:.6f}`")
    lines.append(f"- mean causal-certificate size: `{summary['mean_causal_certificate_size']:.1f}`")
    ...
    lines.append("\n## Interpretation\n")
    lines.append(
        "Phase 46 turns the prior necessity, sufficiency, hostile-coalition, and quorum-lattice checks into a proof-carrying causal certificate. It asks whether the same inferred causal path can be reconstructed as a bundle of agreeing proofs: full witness consensus, leave-one witness replays, minimal clean-enough quorums, and hostile coalition removals. "
...
def save_examples(rng: np.random.Generator, n: int = 4) -> None:
    ...
    for i, fam in enumerate(FAMILIES[:n]):
        ...
        axes[2].set_title("causal-certificate rejection")
        axes[2].axis("equal")
        axes[2].legend(fontsize=8)
        fig.tight_layout()
        fig.savefig(EXAMPLE_DIR / f"phase46_example_{i+1}_{fam}.png", dpi=140)
        plt.close(fig)
...
def main() -> None:
    ...
    print(f"[{PHASE}] reset continued: from leave-one necessity to proof-carrying causal certificate")
    print(f"[{PHASE}] task: infer invariant A->B->C from witnesses, then require the answer branch to survive the proof certificate: full-consensus, minimal-quorum, leave-one, and hostile-coalition replays")

    rows = [run_trial(rng, i, poison_fraction=POISON_FRACTION) for i in range(TRIALS)]
    summary = aggregate(rows)

    trials_path = OUT / "phase46_causal_certificate_verifier_trials.csv"
    summary_path = OUT / "phase46_causal_certificate_verifier_summary.json"
    report_path = OUT / "phase46_causal_certificate_verifier_report.md"
    ...
    plot_hist(rows, "margin", "Phase 46 causal-certificate trajectory answer margin distribution", "runner-up trajectory score - best trajectory score", "phase46_causal_certificate_verifier_margin_distribution.png")
    plot_hist(rows, "final_margin", "Phase 46 final-only endpoint margin distribution", "runner-up endpoint score - best endpoint score", "phase46_causal_certificate_verifier_final_only_margin_distribution.png")
    sweep_rows = poison_sweep()
    write_poison_sweep(sweep_rows)
    save_examples(rng)

    print(f"[{PHASE}] PHASE46_COUNTERFACTUAL_PROOF_CARRYING_CAUSAL_CERTIFICATE_PASS={summary['pass']}")
    print(
    ...
    print(
        ...
        f"mean_causal_certificate_error={summary['mean_causal_certificate_error']:.6f} "
        f"mean_causal_certificate_max_error={summary['mean_causal_certificate_max_error']:.6f} "
        f"mean_causal_certificate_best_error={summary['mean_causal_certificate_best_error']:.6f} "
    ...
    print(f"[{PHASE}] wrote causal-certificate poison sweep: {OUT / 'phase46_causal_certificate_poison_sweep.csv'}")
