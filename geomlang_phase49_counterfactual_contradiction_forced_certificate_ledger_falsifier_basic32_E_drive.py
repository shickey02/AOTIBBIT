...
# -*- coding: utf-8 -*-
"""
Phase 49: Counterfactual contradiction-forced certificate-ledger falsifier
...
Phase 49 adds a negative-control falsification chamber.
...
Writes into E:\BBIT\outputs_basic32 by default:
  ...
  phase49_contradiction_falsifier_trials.csv
  phase49_contradiction_falsifier_summary.json
  phase49_contradiction_falsifier_report.md
  phase49_contradiction_falsifier_poison_sweep.csv
  phase49_examples\*.png
  phase49_contradiction_falsifier_accuracy.png
  phase49_contradiction_falsifier_rejection_rates.png
  phase49_contradiction_falsifier_false_certificate_score_distribution.png
  phase49_contradiction_falsifier_true_vs_false_score_margin.png
  phase49_contradiction_falsifier_branch_counts.png
  phase49_contradiction_falsifier_poison_sweep.png
...
try:
    ...
    plt = None
    print("[49] WARNING: matplotlib unavailable; png plots will be skipped:", exc)


PHASE = "49"
TITLE = "Counterfactual contradiction-forced certificate-ledger falsifier"
PASS_FLAG = "PHASE49_COUNTERFACTUAL_CONTRADICTION_FORCED_CERTIFICATE_LEDGER_FALSIFIER_PASS"
...
def score_certificate(
    ...
    if cert_type != "true_path":
        # We subtract nothing; instead we expose contradiction through higher score.
        score += 0.18 * float(contradiction_penalty / max(1, len(use_indices)))
...
def write_report(path: Path, summary: Dict[str, Any]) -> None:
    ...
    lines.append("")
    lines.append(
        "Phase 49 is a falsification phase. Earlier phases proved that the same branch can survive "
...
def plot_outputs(out_dir: Path, rows: List[Dict[str, Any]], summary: Dict[str, Any]) -> None:
    ...
    plt.ylim(0, 1.08)
    plt.ylabel("rate")
    plt.title("Phase 49 final-only vs true-certificate acceptance and false-certificate rejection")
    plt.legend()
    savefig("phase49_contradiction_falsifier_accuracy.png")
    ...
    plt.title("Phase 49 closest false certificate type")
    savefig("phase49_contradiction_falsifier_rejection_rates.png")
    ...
    plt.ylabel("count")
    plt.title("Phase 49 false certificate score distribution")
    savefig("phase49_contradiction_falsifier_false_certificate_score_distribution.png")
    ...
    plt.ylabel("trials")
    plt.title("Phase 49 true-vs-false certificate selectivity margin")
    savefig("phase49_contradiction_falsifier_true_vs_false_score_margin.png")
    ...
    plt.xticks(rotation=25, ha="right")
    plt.ylabel("trials")
    plt.title("Phase 49 predicted branch counts")
    savefig("phase49_contradiction_falsifier_branch_counts.png")
...
def poison_sweep(args: argparse.Namespace, out_dir: Path) -> List[Dict[str, Any]]:
    ...
    write_csv(out_dir / "phase49_contradiction_falsifier_poison_sweep.csv", sweep_rows)
    ...
    if plt is not None:
        ...
        plt.ylabel("rate")
        plt.ylim(0, 1.08)
        plt.title("Phase 49 poisoned-witness stress sweep")
        plt.legend()
        plt.tight_layout()
        plt.savefig(out_dir / "phase49_contradiction_falsifier_poison_sweep.png", dpi=150)
        plt.close()
...
def write_examples(out_dir: Path, rows: List[Dict[str, Any]]) -> None:
    if plt is None:
        return
    ex_dir = out_dir / "phase49_examples"
...
def main() -> None:
    ...
    print(f"[{PHASE}] reset continued: from certificate-ledger acceptance to contradiction-forced falsification")
    print(f"[{PHASE}] task: accept the true A->B->C certificate while rejecting reversed, shortcut, wrong-middle, wrong-family, and scrambled-order certificates")
    ...
    trials_path = out_dir / "phase49_contradiction_falsifier_trials.csv"
    summary_path = out_dir / "phase49_contradiction_falsifier_summary.json"
    report_path = out_dir / "phase49_contradiction_falsifier_report.md"
    ...
    print(f"[{PHASE}] wrote contradiction-falsifier poison sweep: {out_dir / 'phase49_contradiction_falsifier_poison_sweep.csv'}")
    print(f"[{PHASE}] wrote example png dir: {out_dir / 'phase49_examples'}")
