...
# -*- coding: utf-8 -*-
r"""
Phase 48: Counterfactual cross-examined cross-examined cross-examined certificate-ledger causal audit verifier
...
Phase 48 adds a proof-carrying certificate problem: the system now receives several source
...
Task:
  ...
  Phase 48 adds a cross-examined cross-examined certificate-ledger causal audit check: the consensus is not merely recomputed: the answer is treated as if it must
  carry a compact certificate ledger. The certificate consists of the full
  consensus operator, leave-one witness replay, minimal-quorum replay, and
  hostile-coalition replay. A valid answer must agree with the same branch
  across this proof bundle, proving that the result is not only accurate but
  auditably reconstructable from multiple counterfactual evidence paths.

Outputs:
  E:\BBIT\outputs_basic32\phase48_certificate_ledger_verifier_trials.csv
  E:\BBIT\outputs_basic32\phase48_certificate_ledger_verifier_summary.json
  E:\BBIT\outputs_basic32\phase48_certificate_ledger_verifier_report.md
  E:\BBIT\outputs_basic32\phase48_certificate_ledger_poison_sweep.csv
  E:\BBIT\outputs_basic32\phase48_examples\*.png
...
PHASE = "47"
TITLE = "Counterfactual cross-examined cross-examined cross-examined certificate-ledger causal audit verifier"
SCRIPT_NAME = "geomlang_phase48_counterfactual_proof_carrying_certificate_ledger_verifier_basic32_E_drive.py"
ROOT = Path(r"E:\BBIT") if Path(r"E:\BBIT").exists() else Path.cwd()
OUT = ROOT / "outputs_basic32"
EXAMPLE_DIR = OUT / "phase48_examples"
...
def infer_consensus(witnesses: List[Tuple[np.ndarray, np.ndarray, np.ndarray, bool]]) -> Tuple[Affine, Affine, Dict[str, float]]:
    ...
    # Phase 48 certificate component 1: leave-one replay.
    # The proof bundle should not depend on any single witness.
    leave_one_drifts: List[float] = []
    for held_out in range(len(t1s)):
        keep = [j for j in range(len(t1s)) if j != held_out]
        kr0 = np.asarray([reliab[j] for j in keep], dtype=np.float64)
        l1 = affine_median([t1s[j] for j in keep], kr0)
        l2 = affine_median([t2s[j] for j in keep], kr0)
        drift = (
            np.linalg.norm(c1.A - l1.A) + np.linalg.norm(c1.b - l1.b)
            + np.linalg.norm(c2.A - l2.A) + np.linalg.norm(c2.b - l2.b)
        )
        leave_one_drifts.append(float(drift))

    # Phase 48 cross-examined certificate-ledger sufficiency diagnostic: rebuild the weighted-median
    ...
    # Phase 48 cross-examined certificate-ledger diagnostic: now ask a harder question.
    ...
    # Phase 48 cross-exam ledger diagnostic: interrogate the certificate with
    # every independent witness pair.  A pair is too small to be trusted as a
    # final proof by itself, but the collection of all pairwise replays acts like
    # a cross-examination transcript.  Strong geometry should not depend on one
    # privileged quorum: pairwise reconstructions should cluster around the same
    # causal operators, and the median pair drift becomes an auditable tightness
    # measure for the proof ledger.
    pair_drifts: List[float] = []
    pair_scores: List[float] = []
    pair_size = min(2, len(t1s))
    for pair in combinations(range(len(t1s)), pair_size):
        pr = np.asarray([reliab[j] for j in pair], dtype=np.float64)
        p1 = affine_median([t1s[j] for j in pair], pr)
        p2 = affine_median([t2s[j] for j in pair], pr)
        drift = (
            np.linalg.norm(c1.A - p1.A) + np.linalg.norm(c1.b - p1.b)
            + np.linalg.norm(c2.A - p2.A) + np.linalg.norm(c2.b - p2.b)
        )
        pair_drifts.append(float(drift))
        pair_scores.append(float(np.mean(pr)))

    cross_exam_mean = float(np.mean(pair_drifts))
    cross_exam_median = float(np.median(pair_drifts))
    cross_exam_max = float(np.max(pair_drifts))
    ledger_median = float(np.median(coalition_drifts))
    # High value means the median pair transcript is close to the hostile-coalition
    # certificate and the ledger is not hiding all proof in one witness subset.
    cross_exam_tightness = float(1.0 / (1.0 + abs(cross_exam_median - ledger_median)))

    return c1, c2, {
        "mean_witness_fit": float(np.mean(residuals)),
        "mean_closure_residual": float(np.mean(closure_residuals)),
        "certificate_leave_one_error": float(np.mean(leave_one_drifts)),
        "certificate_leave_one_max_error": float(np.max(leave_one_drifts)),
        ...
        "certificate_ledger_error": float(np.mean(coalition_drifts)),
        "certificate_ledger_max_error": float(np.max(coalition_drifts)),
        "certificate_ledger_best_error": float(np.min(coalition_drifts)),
        "certificate_ledger_size": float(coalition_size),
        "certificate_ledger_mean_fit": float(np.mean(coalition_scores)),
        "cross_exam_pair_error": cross_exam_mean,
        "cross_exam_pair_median_error": cross_exam_median,
        "cross_exam_pair_max_error": cross_exam_max,
        "cross_exam_pair_size": float(pair_size),
        "cross_exam_pair_mean_fit": float(np.mean(pair_scores)),
        "cross_exam_tightness": cross_exam_tightness,
...
def run_trial(rng: np.random.Generator, trial_id: int, poison_fraction: float = POISON_FRACTION) -> Dict[str, object]:
    ...
    # Phase 48 turns the Phase 46 certificate into an audit ledger.  The ledger is
    # intentionally numeric so the CSV stays easy to sort in Excel: a low width means
    # the replay evidence is tight, and a stable best/mean/max ordering means the proof
    # did not depend on a single fragile witness subset.
    row["ledger_error_width"] = float(row["certificate_ledger_max_error"] - row["certificate_ledger_best_error"])
    row["ledger_leave_one_to_lattice_gap"] = float(abs(row["certificate_leave_one_error"] - row["certificate_ledger_error"]))
    row["ledger_tightness"] = float(1.0 / (1.0 + row["ledger_error_width"] + row["ledger_leave_one_to_lattice_gap"]))
    for b in BRANCHES:
...
def aggregate(rows: List[Dict[str, object]]) -> Dict[str, object]:
    ...
    summary: Dict[str, object] = {
        ...
        "mean_certificate_leave_one_error": float(arr("certificate_leave_one_error").mean()),
        "mean_certificate_leave_one_max_error": float(arr("certificate_leave_one_max_error").mean()),
        "mean_certificate_ledger_error": float(arr("certificate_ledger_error").mean()),
        "mean_certificate_ledger_max_error": float(arr("certificate_ledger_max_error").mean()),
        "mean_certificate_ledger_best_error": float(arr("certificate_ledger_best_error").mean()),
        "mean_certificate_ledger_size": float(arr("certificate_ledger_size").mean()),
        "mean_certificate_ledger_fit": float(arr("certificate_ledger_mean_fit").mean()),
        "mean_ledger_error_width": float(arr("ledger_error_width").mean()),
        "mean_ledger_leave_one_to_lattice_gap": float(arr("ledger_leave_one_to_lattice_gap").mean()),
        "mean_ledger_tightness": float(arr("ledger_tightness").mean()),
    ...
    for fam in FAMILIES:
        ...
        fams[fam] = {
            ...
            "mean_certificate_leave_one_error": float(aa("certificate_leave_one_error").mean()),
            "mean_certificate_ledger_error": float(aa("certificate_ledger_error").mean()),
            "mean_ledger_tightness": float(aa("ledger_tightness").mean()),
...
def plot_accuracy(summary: Dict[str, object]) -> None:
    ...
    ax.set_title("Phase 48 counterfactual cross-examined cross-examined certificate-ledger causal audit reasoning: accuracy by hidden family")
    for i, v in enumerate(vals):
        ax.text(v + 0.01, i, f"{v:.2f}", va="center")
    fig.tight_layout()
    fig.savefig(OUT / "phase48_certificate_ledger_verifier_accuracy.png", dpi=140)
    plt.close(fig)
...
def plot_baseline_gap(summary: Dict[str, object]) -> None:
    ...
    ax.bar(x + width/2, traj, width, label="cross-examined cross-examined certificate-ledger verifier")
    ax.set_ylim(0, 1.05)
    ax.set_ylabel("accuracy")
    ax.set_title("Phase 48 final-only baseline vs cross-examined cross-examined certificate-ledger causal audit")
    ax.set_xticks(x)
    ax.set_xticklabels(labels, rotation=22, ha="right")
    ax.legend()
    fig.tight_layout()
    fig.savefig(OUT / "phase48_certificate_ledger_verifier_baseline_gap.png", dpi=140)
    plt.close(fig)
...
def plot_branch_counts(rows: List[Dict[str, object]]) -> None:
    ...
    ax.bar(labels, vals)
    ax.set_ylabel("trials")
    ax.set_title("Phase 48 predicted branch counts")
    ax.tick_params(axis="x", rotation=25)
    fig.tight_layout()
    fig.savefig(OUT / "phase48_certificate_ledger_verifier_branch_counts.png", dpi=140)
    plt.close(fig)
...
def write_poison_sweep(rows: List[Dict[str, float]]) -> None:
    path = OUT / "phase48_certificate_ledger_poison_sweep.csv"
    with path.open("w", newline="", encoding="utf-8") as f:
    ...
    ax.set_ylabel("accuracy")
    ax.set_title("Phase 48 poisoned-witness stress sweep")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(OUT / "phase48_certificate_ledger_verifier_poison_sweep.png", dpi=140)
    plt.close(fig)
...
def write_report(summary: Dict[str, object], path: Path) -> None:
    ...
    lines.append(f"- mean certificate leave-one error: `{summary['mean_certificate_leave_one_error']:.6f}`")
    lines.append(f"- mean certificate leave-one max error: `{summary['mean_certificate_leave_one_max_error']:.6f}`")
    lines.append(f"- mean cross-examined certificate-ledger error: `{summary['mean_certificate_ledger_error']:.6f}`")
    lines.append(f"- mean cross-examined certificate-ledger max error: `{summary['mean_certificate_ledger_max_error']:.6f}`")
    lines.append(f"- mean cross-examined certificate-ledger best error: `{summary['mean_certificate_ledger_best_error']:.6f}`")
    lines.append(f"- mean cross-examined certificate-ledger size: `{summary['mean_certificate_ledger_size']:.1f}`")
    lines.append(f"- mean ledger error width: `{summary['mean_ledger_error_width']:.6f}`")
    lines.append(f"- mean ledger leave-one/lattice gap: `{summary['mean_ledger_leave_one_to_lattice_gap']:.6f}`")
    ...
    lines.append("\n## Interpretation\n")
    lines.append(
        "Phase 48 turns the prior necessity, sufficiency, hostile-coalition, and quorum-lattice checks into a cross-examined cross-examined certificate-ledger causal audit. It asks whether the same inferred causal path can be reconstructed as a bundle of agreeing, ledger-scored proofs: full witness consensus, leave-one witness replays, minimal clean-enough quorums, and hostile coalition removals. "
...
def save_examples(rng: np.random.Generator, n: int = 4) -> None:
    ...
    for i, fam in enumerate(FAMILIES[:n]):
        ...
        axes[2].set_title("cross-examined certificate-ledger rejection")
        axes[2].axis("equal")
        axes[2].legend(fontsize=8)
        fig.tight_layout()
        fig.savefig(EXAMPLE_DIR / f"phase48_example_{i+1}_{fam}.png", dpi=140)
        plt.close(fig)
...
def main() -> None:
    ...
    print(f"[{PHASE}] reset continued: from leave-one necessity to cross-examined cross-examined certificate-ledger causal audit")
    print(f"[{PHASE}] task: infer invariant A->B->C from witnesses, then require the answer branch to survive the audit ledger certificate: full-consensus, leave-one, minimal-quorum, hostile-coalition, monotone audit-ledger, and pairwise cross-examination replay invariants")

    rows = [run_trial(rng, i, poison_fraction=POISON_FRACTION) for i in range(TRIALS)]
    summary = aggregate(rows)

    trials_path = OUT / "phase48_certificate_ledger_verifier_trials.csv"
    summary_path = OUT / "phase48_certificate_ledger_verifier_summary.json"
    report_path = OUT / "phase48_certificate_ledger_verifier_report.md"
    ...
    plot_hist(rows, "margin", "Phase 48 cross-examined certificate-ledger trajectory answer margin distribution", "runner-up trajectory score - best trajectory score", "phase48_certificate_ledger_verifier_margin_distribution.png")
    plot_hist(rows, "final_margin", "Phase 48 final-only endpoint margin distribution", "runner-up endpoint score - best endpoint score", "phase48_certificate_ledger_verifier_final_only_margin_distribution.png")
    plot_hist(rows, "ledger_tightness", "Phase 48 cross-examined certificate-ledger tightness distribution", "ledger tightness", "phase48_certificate_ledger_verifier_ledger_tightness_distribution.png")
    sweep_rows = poison_sweep()
    write_poison_sweep(sweep_rows)
    save_examples(rng)

    print(f"[{PHASE}] PHASE48_COUNTERFACTUAL_CROSS_EXAMINED_CERTIFICATE_LEDGER_PASS={summary['pass']}")
    print(
    ...
    print(
        ...
        f"mean_certificate_leave_one_error={summary['mean_certificate_leave_one_error']:.6f} "
        f"mean_certificate_ledger_error={summary['mean_certificate_ledger_error']:.6f} "
        f"mean_certificate_ledger_max_error={summary['mean_certificate_ledger_max_error']:.6f} "
        f"mean_certificate_ledger_best_error={summary['mean_certificate_ledger_best_error']:.6f} "
        f"mean_ledger_tightness={summary['mean_ledger_tightness']:.6f} "
    ...
    print(f"[{PHASE}] wrote cross-examined certificate-ledger poison sweep: {OUT / 'phase48_certificate_ledger_poison_sweep.csv'}")
