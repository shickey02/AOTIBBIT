...
# -*- coding: utf-8 -*-
r"""
Phase 42: Counterfactual leave-one-witness causal necessity verifier
...
Phase 42 adds a stricter necessity-verification problem: the system now receives several source
...
Task:
  ...
  Phase 42 adds a leave-one-witness necessity check: the consensus is recomputed
...
Outputs:
  E:\BBIT\outputs_basic32\phase42_leave_one_witness_verifier_trials.csv
  E:\BBIT\outputs_basic32\phase42_leave_one_witness_verifier_summary.json
  E:\BBIT\outputs_basic32\phase42_leave_one_witness_verifier_report.md
  E:\BBIT\outputs_basic32\phase42_leave_one_out_poison_sweep.csv
  E:\BBIT\outputs_basic32\phase42_examples\*.png
...
PHASE = "42"
TITLE = "Counterfactual leave-one-witness causal necessity verifier"
SCRIPT_NAME = "geomlang_phase41_counterfactual_holdout_context_causal_abstraction_verifier_basic32_E_drive.py"
ROOT = Path(r"E:\BBIT") if Path(r"E:\BBIT").exists() else Path.cwd()
OUT = ROOT / "outputs_basic32"
EXAMPLE_DIR = OUT / "phase42_examples"
...
def infer_consensus(witnesses: List[Tuple[np.ndarray, np.ndarray, np.ndarray, bool]]) -> Tuple[Affine, Affine, Dict[str, float]]:
    ...
    # Cheap leave-one necessity diagnostic: rebuild the weighted-median
    # consensus with representative witnesses removed and measure operator drift.
    # This avoids a second expensive robust affine fit per holdout while still
    # testing whether the consensus depends on a single witness.
    holdout_indices = sorted(set([0, len(t1s)//2, len(t1s)-1]))
    leave_drifts = []
    for holdout_i in holdout_indices:
        keep_t1 = [x for j, x in enumerate(t1s) if j != holdout_i]
        keep_t2 = [x for j, x in enumerate(t2s) if j != holdout_i]
        keep_r = np.asarray([x for j, x in enumerate(reliab) if j != holdout_i])
        lo1 = affine_median(keep_t1, keep_r)
        lo2 = affine_median(keep_t2, keep_r)
        drift = (
            np.linalg.norm(c1.A - lo1.A) + np.linalg.norm(c1.b - lo1.b)
            + np.linalg.norm(c2.A - lo2.A) + np.linalg.norm(c2.b - lo2.b)
        )
        leave_drifts.append(float(drift))

    return c1, c2, {
        "mean_witness_fit": float(np.mean(residuals)),
        "mean_closure_residual": float(np.mean(closure_residuals)),
        "leave_one_error": float(np.mean(leave_drifts)),
        "leave_one_max_error": float(np.max(leave_drifts)),
...
def make_twin_candidates(
    ...
    # Looks good in the first target context, breaks under the leave-one-witness replay.
...
def aggregate(rows: List[Dict[str, object]]) -> Dict[str, object]:
    ...
    summary: Dict[str, object] = {
        ...
        "mean_leave_one_error": float(arr("leave_one_error").mean()),
        "mean_leave_one_max_error": float(arr("leave_one_max_error").mean()),
    ...
    for fam in FAMILIES:
        ...
        fams[fam] = {
            ...
            "mean_leave_one_error": float(aa("leave_one_error").mean()),
...
def plot_accuracy(summary: Dict[str, object]) -> None:
    ...
    ax.set_title("Phase 42 counterfactual leave-one-witness causal necessity reasoning: accuracy by hidden family")
    for i, v in enumerate(vals):
        ax.text(v + 0.01, i, f"{v:.2f}", va="center")
    fig.tight_layout()
    fig.savefig(OUT / "phase42_leave_one_witness_verifier_accuracy.png", dpi=140)
    plt.close(fig)
...
def plot_baseline_gap(summary: Dict[str, object]) -> None:
    ...
    ax.bar(x + width/2, traj, width, label="leave-one-witness verifier")
    ax.set_ylim(0, 1.05)
    ax.set_ylabel("accuracy")
    ax.set_title("Phase 42 final-only baseline vs leave-one-witness causal necessity")
    ax.set_xticks(x)
    ax.set_xticklabels(labels, rotation=22, ha="right")
    ax.legend()
    fig.tight_layout()
    fig.savefig(OUT / "phase42_leave_one_witness_verifier_baseline_gap.png", dpi=140)
    plt.close(fig)
...
def plot_branch_counts(rows: List[Dict[str, object]]) -> None:
    ...
    ax.bar(labels, vals)
    ax.set_ylabel("trials")
    ax.set_title("Phase 42 predicted branch counts")
    ax.tick_params(axis="x", rotation=25)
    fig.tight_layout()
    fig.savefig(OUT / "phase42_leave_one_witness_verifier_branch_counts.png", dpi=140)
    plt.close(fig)
...
def write_poison_sweep(rows: List[Dict[str, float]]) -> None:
    path = OUT / "phase42_leave_one_out_poison_sweep.csv"
    with path.open("w", newline="", encoding="utf-8") as f:
    ...
    ax.set_ylabel("accuracy")
    ax.set_title("Phase 42 poisoned-witness stress sweep")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(OUT / "phase42_leave_one_witness_verifier_poison_sweep.png", dpi=140)
    plt.close(fig)
...
def write_report(summary: Dict[str, object], path: Path) -> None:
    ...
    lines.append(f"- mean leave-one max error: `{summary['mean_leave_one_max_error']:.6f}`")
    ...
    lines.append("\n## Interpretation\n")
    lines.append(
        "Phase 42 tests whether the path representation remains necessary when individual witnesses are removed from the evidence pool. "
...
def save_examples(rng: np.random.Generator, n: int = 4) -> None:
    ...
    for i, fam in enumerate(FAMILIES[:n]):
        ...
        axes[2].set_title("leave-one-witness rejection")
        axes[2].axis("equal")
        axes[2].legend(fontsize=8)
        fig.tight_layout()
        fig.savefig(EXAMPLE_DIR / f"phase42_example_{i+1}_{fam}.png", dpi=140)
        plt.close(fig)
...
def main() -> None:
    ...
    print(f"[{PHASE}] reset continued: from cross-context bridge reasoning to leave-one-witness causal necessity")
    print(f"[{PHASE}] task: infer invariant A->B->C from witnesses, then require the answer branch to survive a counterfactual leave-one-witness replay")

    rows = [run_trial(rng, i, poison_fraction=POISON_FRACTION) for i in range(TRIALS)]
    summary = aggregate(rows)

    trials_path = OUT / "phase42_leave_one_witness_verifier_trials.csv"
    summary_path = OUT / "phase42_leave_one_witness_verifier_summary.json"
    report_path = OUT / "phase42_leave_one_witness_verifier_report.md"
    ...
    plot_hist(rows, "margin", "Phase 42 leave-one-witness trajectory answer margin distribution", "runner-up trajectory score - best trajectory score", "phase42_leave_one_witness_verifier_margin_distribution.png")
    plot_hist(rows, "final_margin", "Phase 42 final-only endpoint margin distribution", "runner-up endpoint score - best endpoint score", "phase42_leave_one_witness_verifier_final_only_margin_distribution.png")
    sweep_rows = poison_sweep()
    write_poison_sweep(sweep_rows)
    save_examples(rng)

    print(f"[{PHASE}] PHASE42_COUNTERFACTUAL_LEAVE_ONE_WITNESS_CAUSAL_NECESSITY_PASS={summary['pass']}")
    print(
    ...
    print(
        ...
        f"mean_leave_one_max_error={summary['mean_leave_one_max_error']:.6f} "
    ...
    print(f"[{PHASE}] wrote leave-one-out poison sweep: {OUT / 'phase42_leave_one_out_poison_sweep.csv'}")
