...
# -*- coding: utf-8 -*-
"""
Phase 50: Counterfactual axis-typed contradiction-forced certificate-ledger falsifier

Drop-in path:
  E:\BBIT\bbit_geomlang\geomlang_phase49_counterfactual_contradiction_forced_certificate_ledger_falsifier_basic32_E_drive.py

Run:
  python bbit_geomlang/geomlang_phase49_counterfactual_contradiction_forced_certificate_ledger_falsifier_basic32_E_drive.py

Purpose
-------
Phase 48 proved the certificate-ledger path can remain stable under witness deletion,
minimal quorums, hostile coalitions, and cross-examination. But the Phase 48 run also
...
Phase 50 adds a negative-control falsification chamber.
...
Writes into E:\BBIT\outputs_basic32 by default:
  ...
  phase50_axis_typed_contradiction_falsifier_trials.csv
  phase50_axis_typed_contradiction_falsifier_summary.json
  phase50_axis_typed_contradiction_falsifier_report.md
  phase50_axis_typed_contradiction_falsifier_poison_sweep.csv
  phase50_examples\*.png
  phase50_axis_typed_contradiction_falsifier_accuracy.png
  phase50_axis_typed_contradiction_falsifier_rejection_rates.png
  phase50_axis_typed_contradiction_falsifier_false_certificate_score_distribution.png
  phase50_axis_typed_contradiction_falsifier_true_vs_false_score_margin.png
  phase50_axis_typed_contradiction_falsifier_branch_counts.png
  phase50_axis_typed_contradiction_falsifier_poison_sweep.png
...
try:
    ...
    plt = None
    print("[50] WARNING: matplotlib unavailable; png plots will be skipped:", exc)


PHASE = "50"
TITLE = "Counterfactual axis-typed contradiction-forced certificate-ledger falsifier"
PASS_FLAG = "PHASE50_COUNTERFACTUAL_AXIS_TYPED_CONTRADICTION_FORCED_CERTIFICATE_LEDGER_FALSIFIER_PASS"
...
def write_report(path: Path, summary: Dict[str, Any]) -> None:
    ...
    lines.append("")
    lines.append(
        "Phase 50 is a falsification phase. Earlier phases proved that the same branch can survive "
    ...
    lines.append("")
    lines.append(
        "A clean pass means the system is no longer only preserving a robust path; it is also showing selectivity. "
...
def plot_outputs(out_dir: Path, rows: List[Dict[str, Any]], summary: Dict[str, Any]) -> None:
    ...
    plt.ylim(0, 1.08)
    plt.ylabel("rate")
    plt.title("Phase 50 final-only vs true-certificate acceptance and false-certificate rejection")
    plt.legend()
    savefig("phase50_axis_typed_contradiction_falsifier_accuracy.png")
    ...
    plt.title("Phase 50 closest false certificate type")
    savefig("phase50_axis_typed_contradiction_falsifier_rejection_rates.png")
    ...
    plt.ylabel("count")
    plt.title("Phase 50 false certificate score distribution")
    savefig("phase50_axis_typed_contradiction_falsifier_false_certificate_score_distribution.png")
    ...
    plt.ylabel("trials")
    plt.title("Phase 50 true-vs-false certificate selectivity margin")
    savefig("phase50_axis_typed_contradiction_falsifier_true_vs_false_score_margin.png")
    ...
    plt.xticks(rotation=25, ha="right")
    plt.ylabel("trials")
    plt.title("Phase 50 predicted branch counts")
    savefig("phase50_axis_typed_contradiction_falsifier_branch_counts.png")
...
def poison_sweep(args: argparse.Namespace, out_dir: Path) -> List[Dict[str, Any]]:
    ...
    write_csv(out_dir / "phase50_axis_typed_contradiction_falsifier_poison_sweep.csv", sweep_rows)
    ...
    if plt is not None:
        ...
        plt.ylabel("rate")
        plt.ylim(0, 1.08)
        plt.title("Phase 50 poisoned-witness stress sweep")
        plt.legend()
        plt.tight_layout()
        plt.savefig(out_dir / "phase50_axis_typed_contradiction_falsifier_poison_sweep.png", dpi=150)
        plt.close()
...
def write_examples(out_dir: Path, rows: List[Dict[str, Any]]) -> None:
    if plt is None:
        return
    ex_dir = out_dir / "phase50_examples"
    ...
    for r in selected:
        ...
        plt.tight_layout()
        plt.savefig(ex_dir / f"phase49_trial_{int(r['trial']):04d}_certificate_scores.png", dpi=140)
        plt.close()
...
def main() -> None:
    ...
    print(f"[{PHASE}] reset continued: from untyped contradiction falsification to axis-typed role contradiction falsification")
    print(f"[{PHASE}] task: accept the true A->B->C certificate while rejecting false certificates through typed A/B/C role signatures, axis-aware transition probes, and contradiction margins")
    ...
    trials_path = out_dir / "phase50_axis_typed_contradiction_falsifier_trials.csv"
    summary_path = out_dir / "phase50_axis_typed_contradiction_falsifier_summary.json"
    report_path = out_dir / "phase50_axis_typed_contradiction_falsifier_report.md"
    ...
    print(f"[{PHASE}] wrote contradiction-falsifier poison sweep: {out_dir / 'phase50_axis_typed_contradiction_falsifier_poison_sweep.csv'}")
    print(f"[{PHASE}] wrote example png dir: {out_dir / 'phase50_examples'}")
