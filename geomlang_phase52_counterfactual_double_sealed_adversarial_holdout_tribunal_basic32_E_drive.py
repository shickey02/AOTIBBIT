...
# -*- coding: utf-8 -*-
"""
Phase 52: Counterfactual double-sealed adversarial holdout tribunal

Drop-in path:
  E:\BBIT\bbit_geomlang\geomlang_phase52_counterfactual_double_sealed_adversarial_holdout_tribunal_basic32_E_drive.py

Run:
  python bbit_geomlang/geomlang_phase52_counterfactual_double_sealed_adversarial_holdout_tribunal_basic32_E_drive.py

Purpose
-------
Phase 51 proved the typed contradiction tribunal could accept the true A -> B -> C certificate,
reject the false certificates, and pass one sealed holdout check. Phase 52 raises the bar by
adding a double-sealed adversarial holdout tribunal: the certificate must keep its selectivity
on two independent panels, including a higher-noise adversarial panel that was never used to
select the winning branch.

Phase 52 adds this double-sealed holdout tribunal on top of the Phase 51 typed falsifier.
...
Writes into E:\BBIT\outputs_basic32 by default:
  ...
  phase52_double_sealed_adversarial_holdout_tribunal_trials.csv
  phase52_double_sealed_adversarial_holdout_tribunal_summary.json
  phase52_double_sealed_adversarial_holdout_tribunal_report.md
  phase52_double_sealed_adversarial_holdout_tribunal_poison_sweep.csv
  phase52_examples\*.png
  phase52_double_sealed_adversarial_holdout_tribunal_accuracy.png
  phase52_double_sealed_adversarial_holdout_tribunal_rejection_rates.png
  phase52_double_sealed_adversarial_holdout_tribunal_false_certificate_score_distribution.png
  phase52_double_sealed_adversarial_holdout_tribunal_true_vs_false_score_margin.png
  phase52_double_sealed_adversarial_holdout_tribunal_branch_counts.png
  phase52_double_sealed_adversarial_holdout_tribunal_poison_sweep.png
...
try:
    ...
    plt = None
    print("[52] WARNING: matplotlib unavailable; png plots will be skipped:", exc)


PHASE = "52"
TITLE = "Counterfactual double-sealed adversarial holdout tribunal"
PASS_FLAG = "PHASE52_COUNTERFACTUAL_DOUBLE_SEALED_ADVERSARIAL_HOLDOUT_TRIBUNAL_PASS"
...
def run_trial(
    ...
    # Phase 52: double-sealed adversarial holdout tribunal.
    ...
    # lightweight: Phase 52 should not multiply the full certificate grid again; it
    ...
    double_sealed_pass = bool(double_sealed_margin > 0.001 and trajectory_correct and false_rejected)
...
def write_report(path: Path, summary: Dict[str, Any]) -> None:
    ...
    lines.append("")
    lines.append("| family | traj_acc | final_acc | stable | false_reject | double_sealed | holdout_margin | false_margin | ledger_tight | min_margin |")
    lines.append("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
    for fam, s in summary["families"].items():
        lines.append(
            ...
            f"{s['double_sealed_acceptance_rate']:.3f} | {s['mean_double_sealed_margin']:.6f} | "
            f"{s['mean_false_margin']:.6f} | {s['mean_ledger_tightness']:.6f} | {s['min_margin']:.6f} |"
        )
    lines.append("")
    lines.append("## Interpretation")
    lines.append("")
    lines.append(
        "Phase 52 is a falsification phase. Earlier phases proved that the same branch can survive "
    ...
    lines.append("")
    lines.append(
        "A clean pass means the system is no longer only preserving a robust path; it is also showing sealed holdout selectivity. "
...
def plot_outputs(out_dir: Path, rows: List[Dict[str, Any]], summary: Dict[str, Any]) -> None:
    ...
    plt.ylim(0, 1.08)
    plt.ylabel("rate")
    plt.title("Phase 52 final-only vs true-certificate acceptance and false-certificate rejection")
    plt.legend()
    savefig("phase52_double_sealed_adversarial_holdout_tribunal_accuracy.png")
    ...
    plt.title("Phase 52 closest false certificate type")
    savefig("phase52_double_sealed_adversarial_holdout_tribunal_rejection_rates.png")
    ...
    plt.ylabel("count")
    plt.title("Phase 52 false certificate score distribution")
    savefig("phase52_double_sealed_adversarial_holdout_tribunal_false_certificate_score_distribution.png")
    ...
    plt.ylabel("trials")
    plt.title("Phase 52 true-vs-false certificate selectivity margin")
    savefig("phase52_double_sealed_adversarial_holdout_tribunal_true_vs_false_score_margin.png")
    ...
    plt.title("Phase 52 sealed holdout tribunal pass rate by family")
    savefig("phase52_double_sealed_adversarial_holdout_tribunal_holdout_pass_rate.png")
    ...
    plt.xticks(rotation=25, ha="right")
    plt.ylabel("trials")
    plt.title("Phase 52 predicted branch counts")
    savefig("phase52_double_sealed_adversarial_holdout_tribunal_branch_counts.png")
...
def poison_sweep(args: argparse.Namespace, out_dir: Path) -> List[Dict[str, Any]]:
    ...
    write_csv(out_dir / "phase52_double_sealed_adversarial_holdout_tribunal_poison_sweep.csv", sweep_rows)
    ...
    if plt is not None:
        ...
        plt.ylabel("rate")
        plt.ylim(0, 1.08)
        plt.title("Phase 52 poisoned-witness stress sweep")
        plt.legend()
        plt.tight_layout()
        plt.savefig(out_dir / "phase52_double_sealed_adversarial_holdout_tribunal_poison_sweep.png", dpi=150)
        plt.close()
...
def write_examples(out_dir: Path, rows: List[Dict[str, Any]]) -> None:
    if plt is None:
        return
    ex_dir = out_dir / "phase52_examples"
    ...
    for r in selected:
        ...
        plt.tight_layout()
        plt.savefig(ex_dir / f"phase52_trial_{int(r['trial']):04d}_certificate_scores.png", dpi=140)
        plt.close()
...
def main() -> None:
    ...
    print(f"[{PHASE}] reset continued: from sealed holdout contradiction tribunal to double-sealed adversarial holdout tribunal")
    print(f"[{PHASE}] task: accept the true A->B->C certificate while rejecting false certificates through typed A/B/C role signatures, axis-aware transition probes, contradiction margins, and a double-sealed consistency tribunal and adversarial holdout mutation challenge")
    ...
    trials_path = out_dir / "phase52_double_sealed_adversarial_holdout_tribunal_trials.csv"
    summary_path = out_dir / "phase52_double_sealed_adversarial_holdout_tribunal_summary.json"
    report_path = out_dir / "phase52_double_sealed_adversarial_holdout_tribunal_report.md"
    ...
    print(
        ...
        f"sealed_holdout_acceptance_rate={summary['sealed_holdout_acceptance_rate']:.4f} double_sealed_acceptance_rate={summary['double_sealed_acceptance_rate']:.4f} "
    ...
    print(f"[{PHASE}] wrote contradiction-falsifier poison sweep: {out_dir / 'phase52_double_sealed_adversarial_holdout_tribunal_poison_sweep.csv'}")
    print(f"[{PHASE}] wrote example png dir: {out_dir / 'phase52_examples'}")
