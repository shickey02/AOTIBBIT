...
# -*- coding: utf-8 -*-
"""
Phase 54: Counterfactual null-ablation near-miss adversarial tribunal

Drop-in path:
  E:\BBIT\bbit_geomlang\geomlang_phase54_counterfactual_null_ablation_near_miss_adversarial_tribunal_basic32_E_drive.py

Run:
  python bbit_geomlang/geomlang_phase54_counterfactual_null_ablation_near_miss_adversarial_tribunal_basic32_E_drive.py

Purpose
-------
Phase 51 proved the typed contradiction tribunal could accept the true A -> B -> C certificate,
reject the false certificates, and pass one sealed holdout check. Phase 54 raises the bar by
adding a null-ablation near-miss adversarial tribunal: the certificate must keep its selectivity
on two independent panels, including a higher-noise adversarial panel that was never used to
select the winning branch.

Phase 54 adds this double-sealed holdout tribunal on top of the Phase 51 typed falsifier.
...
Writes into E:\BBIT\outputs_basic32 by default:
  ...
  phase54_null_ablation_near_miss_adversarial_tribunal_trials.csv
  phase54_null_ablation_near_miss_adversarial_tribunal_summary.json
  phase54_null_ablation_near_miss_adversarial_tribunal_report.md
  phase54_null_ablation_near_miss_adversarial_tribunal_poison_sweep.csv
  phase54_examples\*.png
  phase54_null_ablation_near_miss_adversarial_tribunal_accuracy.png
  phase54_null_ablation_near_miss_adversarial_tribunal_rejection_rates.png
  phase54_null_ablation_near_miss_adversarial_tribunal_false_certificate_score_distribution.png
  phase54_null_ablation_near_miss_adversarial_tribunal_true_vs_false_score_margin.png
  phase54_null_ablation_near_miss_adversarial_tribunal_branch_counts.png
  phase54_null_ablation_near_miss_adversarial_tribunal_poison_sweep.png
...
try:
    ...
    plt = None
    print("[54] WARNING: matplotlib unavailable; png plots will be skipped:", exc)


PHASE = "54"
TITLE = "Counterfactual null-ablation near-miss adversarial tribunal"
PASS_FLAG = "PHASE54_COUNTERFACTUAL_NULL_ABLATION_NEAR_MISS_ADVERSARIAL_TRIBUNAL_PASS"
...
def run_trial(
    ...
    # Phase 54 calibration layer.  Phase 52 proved that false certificates can be
    ...
    # Phase 54: null-ablation near-miss adversarial tribunal.
    ...
    # lightweight: Phase 54 should not multiply the full certificate grid again; it
...
def summarize(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # Phase 54 adds a null-ablation jury: the certificate must remain good when
    ...
    for fam in FAMILIES:
        ...
        fams[fam] = {
            ...
            "null_ablation_jury_rate": float(np.mean([1.0 if (r["trajectory_correct"] and r["calibrated_false_rejected"] and r["double_sealed_pass"] and r["near_miss_margin"] > 0.01 and r["relative_false_margin"] > 0.01 and r["log_false_margin"] > 0.01) else 0.0 for r in sub])),
...
def write_report(path: Path, summary: Dict[str, Any]) -> None:
    ...
    lines.append(f"- null-ablation jury rate: `{summary['null_ablation_jury_rate']:.4f}`")
    ...
    lines.append("")
    lines.append(
        "Phase 54 is a falsification phase. Earlier phases proved that the same branch can survive "
...
def plot_outputs(out_dir: Path, rows: List[Dict[str, Any]], summary: Dict[str, Any]) -> None:
    ...
    plt.ylim(0, 1.08)
    plt.ylabel("rate")
    plt.title("Phase 54 final-only vs true-certificate acceptance and false-certificate rejection")
    plt.legend()
    savefig("phase54_null_ablation_near_miss_adversarial_tribunal_accuracy.png")
    ...
    plt.title("Phase 54 closest false certificate type")
    savefig("phase54_null_ablation_near_miss_adversarial_tribunal_rejection_rates.png")
    ...
    plt.ylabel("count")
    plt.title("Phase 54 false certificate score distribution")
    savefig("phase54_null_ablation_near_miss_adversarial_tribunal_false_certificate_score_distribution.png")
    ...
    plt.ylabel("trials")
    plt.title("Phase 54 true-vs-false certificate selectivity margin")
    savefig("phase54_null_ablation_near_miss_adversarial_tribunal_true_vs_false_score_margin.png")
    ...
    plt.ylabel("trials")
    plt.title("Phase 54 calibrated near-miss selectivity margin")
    savefig("phase54_null_ablation_near_miss_adversarial_tribunal_near_miss_margin.png")
    ...
    plt.ylabel("count")
    plt.title("Phase 54 log-scaled false certificate score distribution")
    savefig("phase54_null_ablation_near_miss_adversarial_tribunal_false_certificate_log_score_distribution.png")
    ...
    plt.title("Phase 54 sealed holdout tribunal pass rate by family")
    savefig("phase54_null_ablation_near_miss_adversarial_tribunal_holdout_pass_rate.png")
    ...
    plt.xticks(rotation=25, ha="right")
    plt.ylabel("trials")
    plt.title("Phase 54 predicted branch counts")
    savefig("phase54_null_ablation_near_miss_adversarial_tribunal_branch_counts.png")
...
def poison_sweep(args: argparse.Namespace, out_dir: Path) -> List[Dict[str, Any]]:
    ...
    write_csv(out_dir / "phase54_null_ablation_near_miss_adversarial_tribunal_poison_sweep.csv", sweep_rows)
    ...
    if plt is not None:
        ...
        plt.ylabel("rate")
        plt.ylim(0, 1.08)
        plt.title("Phase 54 poisoned-witness stress sweep")
        plt.legend()
        plt.tight_layout()
        plt.savefig(out_dir / "phase54_null_ablation_near_miss_adversarial_tribunal_poison_sweep.png", dpi=150)
        plt.close()
...
def write_examples(out_dir: Path, rows: List[Dict[str, Any]]) -> None:
    if plt is None:
        return
    ex_dir = out_dir / "phase53_examples"
    ...
    for r in selected:
        ...
        plt.tight_layout()
        plt.savefig(ex_dir / f"phase53_trial_{int(r['trial']):04d}_certificate_scores.png", dpi=140)
        plt.close()
...
def main() -> None:
    ...
    print(f"[{PHASE}] reset continued: from sealed holdout contradiction tribunal to null-ablation near-miss adversarial tribunal")
    print(f"[{PHASE}] task: accept the true A->B->C certificate while rejecting false certificates through typed A/B/C role signatures, axis-aware transition probes, contradiction margins, and a null-ablation jury that removes each evidence strut in turn, tests scale-normalized false margins, hard negative pressure, adversarial near-miss separation, and refuses proofs that only work because one cue dominates")
    ...
    trials_path = out_dir / "phase54_null_ablation_near_miss_adversarial_tribunal_trials.csv"
    summary_path = out_dir / "phase54_null_ablation_near_miss_adversarial_tribunal_summary.json"
    report_path = out_dir / "phase54_null_ablation_near_miss_adversarial_tribunal_report.md"
    ...
    print(
        ...
        f"sealed_holdout_margin_floor={summary['sealed_holdout_margin_floor']:.6f} double_sealed_margin_floor={summary['double_sealed_margin_floor']:.6f}"
    )
    ...
    for fam, s in summary["families"].items():
        print(
            ...
            f"jury={s['null_ablation_jury_rate']:.3f} "
    ...
    print(f"[{PHASE}] wrote contradiction-falsifier poison sweep: {out_dir / 'phase54_null_ablation_near_miss_adversarial_tribunal_poison_sweep.csv'}")
    print(f"[{PHASE}] wrote example png dir: {out_dir / 'phase54_examples'}")
