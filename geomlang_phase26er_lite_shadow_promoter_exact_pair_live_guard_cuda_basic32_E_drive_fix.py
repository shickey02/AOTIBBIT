...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    # 26ER-HOTFIX:
    # The first ER build correctly found a promotion-ready selected target, but it
    # refused --apply because promotion_plan_ready was calculated globally across
    # every shadow candidate. That was too strict: non-selected diagnostic shadows
    # may lack guard tokens or fail syntax without invalidating the selected target.
    # Promotion readiness must be selected-target readiness, while global failures
    # remain diagnostic fields in the manifest.
    selected_ready = bool(selected and selected.get("promotion_ready"))
    selected_syntax_ok = bool(selected and selected.get("shadow_syntax_ok"))
    selected_guard_ok = bool(selected and selected.get("guard_tokens_present"))
    promotion_plan_ready = bool(
        eq_ready
        and exact_pair_pass
        and metric_pass
        and selected_ready
        and selected_syntax_ok
        and selected_guard_ok
    )
