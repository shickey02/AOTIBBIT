...
# -*- coding: utf-8 -*-
"""
Phase 53: Counterfactual calibrated near-miss adversarial tribunal

Drop-in path:
  E:\BBIT\bbit_geomlang\geomlang_phase53_counterfactual_calibrated_near_miss_adversarial_tribunal_basic32_E_drive.py

Run:
  python bbit_geomlang/geomlang_phase53_counterfactual_calibrated_near_miss_adversarial_tribunal_basic32_E_drive.py

Purpose
-------
Phase 51 proved the typed contradiction tribunal could accept the true A -> B -> C certificate,
reject the false certificates, and pass one sealed holdout check. Phase 53 raises the bar by
adding a calibrated near-miss adversarial tribunal: the certificate must keep its selectivity
on two independent panels, including a higher-noise adversarial panel that was never used to
select the winning branch.

Phase 53 adds this double-sealed holdout tribunal on top of the Phase 51 typed falsifier.
...
Writes into E:\BBIT\outputs_basic32 by default:
  ...
  phase53_calibrated_near_miss_adversarial_tribunal_trials.csv
  phase53_calibrated_near_miss_adversarial_tribunal_summary.json
  phase53_calibrated_near_miss_adversarial_tribunal_report.md
  phase53_calibrated_near_miss_adversarial_tribunal_poison_sweep.csv
  phase52_examples\*.png
  phase53_calibrated_near_miss_adversarial_tribunal_accuracy.png
  phase53_calibrated_near_miss_adversarial_tribunal_rejection_rates.png
  phase53_calibrated_near_miss_adversarial_tribunal_false_certificate_score_distribution.png
  phase53_calibrated_near_miss_adversarial_tribunal_true_vs_false_score_margin.png
  phase53_calibrated_near_miss_adversarial_tribunal_branch_counts.png
  phase53_calibrated_near_miss_adversarial_tribunal_poison_sweep.png
...
try:
    ...
    plt = None
    print("[53] WARNING: matplotlib unavailable; png plots will be skipped:", exc)


PHASE = "52"
TITLE = "Counterfactual calibrated near-miss adversarial tribunal"
PASS_FLAG = "PHASE53_COUNTERFACTUAL_CALIBRATED_NEAR_MISS_ADVERSARIAL_TRIBUNAL_PASS"
...
def run_trial(
    ...
    # Phase 53 calibration layer.  Phase 52 proved that false certificates can be
    ...
    # Phase 53: calibrated near-miss adversarial tribunal.
    ...
    # lightweight: Phase 53 should not multiply the full certificate grid again; it
...
def write_report(path: Path, summary: Dict[str, Any]) -> None:
    ...
    lines.append("")
    lines.append(
        "Phase 53 is a falsification phase. Earlier phases proved that the same branch can survive "
...
def plot_outputs(out_dir: Path, rows: List[Dict[str, Any]], summary: Dict[str, Any]) -> None:
    ...
    plt.ylim(0, 1.08)
    plt.ylabel("rate")
    plt.title("Phase 53 final-only vs true-certificate acceptance and false-certificate rejection")
    plt.legend()
    savefig("phase53_calibrated_near_miss_adversarial_tribunal_accuracy.png")
    ...
    plt.title("Phase 53 closest false certificate type")
    savefig("phase53_calibrated_near_miss_adversarial_tribunal_rejection_rates.png")
    ...
    plt.ylabel("count")
    plt.title("Phase 53 false certificate score distribution")
    savefig("phase53_calibrated_near_miss_adversarial_tribunal_false_certificate_score_distribution.png")
    ...
    plt.ylabel("trials")
    plt.title("Phase 53 true-vs-false certificate selectivity margin")
    savefig("phase53_calibrated_near_miss_adversarial_tribunal_true_vs_false_score_margin.png")
    ...
    plt.ylabel("trials")
    plt.title("Phase 53 calibrated near-miss selectivity margin")
    savefig("phase53_calibrated_near_miss_adversarial_tribunal_near_miss_margin.png")
    ...
    plt.ylabel("count")
    plt.title("Phase 53 log-scaled false certificate score distribution")
    savefig("phase53_calibrated_near_miss_adversarial_tribunal_false_certificate_log_score_distribution.png")
    ...
    plt.title("Phase 53 sealed holdout tribunal pass rate by family")
    savefig("phase53_calibrated_near_miss_adversarial_tribunal_holdout_pass_rate.png")
    ...
    plt.xticks(rotation=25, ha="right")
    plt.ylabel("trials")
    plt.title("Phase 53 predicted branch counts")
    savefig("phase53_calibrated_near_miss_adversarial_tribunal_branch_counts.png")
...
def poison_sweep(args: argparse.Namespace, out_dir: Path) -> List[Dict[str, Any]]:
    ...
    write_csv(out_dir / "phase53_calibrated_near_miss_adversarial_tribunal_poison_sweep.csv", sweep_rows)
    ...
    if plt is not None:
        ...
        plt.ylabel("rate")
        plt.ylim(0, 1.08)
        plt.title("Phase 53 poisoned-witness stress sweep")
        plt.legend()
        plt.tight_layout()
        plt.savefig(out_dir / "phase53_calibrated_near_miss_adversarial_tribunal_poison_sweep.png", dpi=150)
        plt.close()
...
def main() -> None:
    ...
    print(f"[{PHASE}] reset continued: from sealed holdout contradiction tribunal to calibrated near-miss adversarial tribunal")
    print(f"[{PHASE}] task: accept the true A->B->C certificate while rejecting false certificates through typed A/B/C role signatures, axis-aware transition probes, contradiction margins, and a calibrated near-miss tribunal that tests scale-normalized false margins, hard negative pressure, and adversarial near-miss separation")
    ...
    trials_path = out_dir / "phase53_calibrated_near_miss_adversarial_tribunal_trials.csv"
    summary_path = out_dir / "phase53_calibrated_near_miss_adversarial_tribunal_summary.json"
    report_path = out_dir / "phase53_calibrated_near_miss_adversarial_tribunal_report.md"
    ...
    print(f"[{PHASE}] wrote contradiction-falsifier poison sweep: {out_dir / 'phase53_calibrated_near_miss_adversarial_tribunal_poison_sweep.csv'}")
