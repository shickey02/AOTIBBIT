...
# -*- coding: utf-8 -*-
r"""
Phase 47: Counterfactual certificate-ledger causal audit verifier
...
Phase 47 adds a proof-carrying certificate problem: the system now receives several source
...
Task:
  ...
  Phase 47 adds a certificate-ledger causal audit check: the consensus is not merely recomputed: the answer is treated as if it must
...
Outputs:
  E:\BBIT\outputs_basic32\phase47_certificate_ledger_verifier_trials.csv
  E:\BBIT\outputs_basic32\phase47_certificate_ledger_verifier_summary.json
  E:\BBIT\outputs_basic32\phase47_certificate_ledger_verifier_report.md
  E:\BBIT\outputs_basic32\phase47_certificate_ledger_poison_sweep.csv
  E:\BBIT\outputs_basic32\phase47_examples\*.png
...
PHASE = "47"
TITLE = "Counterfactual certificate-ledger causal audit verifier"
SCRIPT_NAME = "geomlang_phase47_counterfactual_proof_carrying_certificate_ledger_verifier_basic32_E_drive.py"
ROOT = Path(r"E:\BBIT") if Path(r"E:\BBIT").exists() else Path.cwd()
OUT = ROOT / "outputs_basic32"
EXAMPLE_DIR = OUT / "phase47_examples"
...
def infer_consensus(witnesses: List[Tuple[np.ndarray, np.ndarray, np.ndarray, bool]]) -> Tuple[Affine, Affine, Dict[str, float]]:
    ...
    # Phase 47 certificate component 1: leave-one replay.
    ...
    # Phase 47 certificate-ledger sufficiency diagnostic: rebuild the weighted-median
    ...
    # Phase 47 certificate-ledger diagnostic: now ask a harder question.
...
def run_trial(rng: np.random.Generator, trial_id: int, poison_fraction: float = POISON_FRACTION) -> Dict[str, object]:
    ...
    # Phase 47 turns the Phase 46 certificate into an audit ledger.  The ledger is
...
def plot_accuracy(summary: Dict[str, object]) -> None:
    ...
    ax.set_title("Phase 47 counterfactual certificate-ledger causal audit reasoning: accuracy by hidden family")
    for i, v in enumerate(vals):
        ax.text(v + 0.01, i, f"{v:.2f}", va="center")
    fig.tight_layout()
    fig.savefig(OUT / "phase47_certificate_ledger_verifier_accuracy.png", dpi=140)
    plt.close(fig)
...
def plot_baseline_gap(summary: Dict[str, object]) -> None:
    ...
    ax.bar(x + width/2, traj, width, label="certificate-ledger verifier")
    ax.set_ylim(0, 1.05)
    ax.set_ylabel("accuracy")
    ax.set_title("Phase 47 final-only baseline vs certificate-ledger causal audit")
    ax.set_xticks(x)
    ax.set_xticklabels(labels, rotation=22, ha="right")
    ax.legend()
    fig.tight_layout()
    fig.savefig(OUT / "phase47_certificate_ledger_verifier_baseline_gap.png", dpi=140)
    plt.close(fig)
...
def plot_branch_counts(rows: List[Dict[str, object]]) -> None:
    ...
    ax.bar(labels, vals)
    ax.set_ylabel("trials")
    ax.set_title("Phase 47 predicted branch counts")
    ax.tick_params(axis="x", rotation=25)
    fig.tight_layout()
    fig.savefig(OUT / "phase47_certificate_ledger_verifier_branch_counts.png", dpi=140)
    plt.close(fig)
...
def write_poison_sweep(rows: List[Dict[str, float]]) -> None:
    path = OUT / "phase47_certificate_ledger_poison_sweep.csv"
    with path.open("w", newline="", encoding="utf-8") as f:
    ...
    ax.set_ylabel("accuracy")
    ax.set_title("Phase 47 poisoned-witness stress sweep")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(OUT / "phase47_certificate_ledger_verifier_poison_sweep.png", dpi=140)
    plt.close(fig)
...
def write_report(summary: Dict[str, object], path: Path) -> None:
    ...
    lines.append(f"- mean certificate-ledger error: `{summary['mean_certificate_ledger_error']:.6f}`")
    lines.append(f"- mean certificate-ledger max error: `{summary['mean_certificate_ledger_max_error']:.6f}`")
    lines.append(f"- mean certificate-ledger best error: `{summary['mean_certificate_ledger_best_error']:.6f}`")
    lines.append(f"- mean certificate-ledger size: `{summary['mean_certificate_ledger_size']:.1f}`")
    ...
    lines.append("\n## Interpretation\n")
    lines.append(
        "Phase 47 turns the prior necessity, sufficiency, hostile-coalition, and quorum-lattice checks into a certificate-ledger causal audit. It asks whether the same inferred causal path can be reconstructed as a bundle of agreeing, ledger-scored proofs: full witness consensus, leave-one witness replays, minimal clean-enough quorums, and hostile coalition removals. "
...
def save_examples(rng: np.random.Generator, n: int = 4) -> None:
    ...
    for i, fam in enumerate(FAMILIES[:n]):
        ...
        axes[2].set_title("certificate-ledger rejection")
        axes[2].axis("equal")
        axes[2].legend(fontsize=8)
        fig.tight_layout()
        fig.savefig(EXAMPLE_DIR / f"phase47_example_{i+1}_{fam}.png", dpi=140)
        plt.close(fig)
...
def main() -> None:
    ...
    print(f"[{PHASE}] reset continued: from leave-one necessity to certificate-ledger causal audit")
    print(f"[{PHASE}] task: infer invariant A->B->C from witnesses, then require the answer branch to survive the audit ledger certificate: full-consensus, leave-one, minimal-quorum, hostile-coalition, and monotone audit-ledger replay invariants")

    rows = [run_trial(rng, i, poison_fraction=POISON_FRACTION) for i in range(TRIALS)]
    summary = aggregate(rows)

    trials_path = OUT / "phase47_certificate_ledger_verifier_trials.csv"
    summary_path = OUT / "phase47_certificate_ledger_verifier_summary.json"
    report_path = OUT / "phase47_certificate_ledger_verifier_report.md"
    ...
    plot_hist(rows, "margin", "Phase 47 certificate-ledger trajectory answer margin distribution", "runner-up trajectory score - best trajectory score", "phase47_certificate_ledger_verifier_margin_distribution.png")
    plot_hist(rows, "final_margin", "Phase 47 final-only endpoint margin distribution", "runner-up endpoint score - best endpoint score", "phase47_certificate_ledger_verifier_final_only_margin_distribution.png")
    plot_hist(rows, "ledger_tightness", "Phase 47 certificate-ledger tightness distribution", "ledger tightness", "phase47_certificate_ledger_verifier_ledger_tightness_distribution.png")
    sweep_rows = poison_sweep()
    write_poison_sweep(sweep_rows)
    save_examples(rng)

    print(f"[{PHASE}] PHASE47_COUNTERFACTUAL_PROOF_CARRYING_CERTIFICATE_LEDGER_PASS={summary['pass']}")
    print(
    ...
    print(f"[{PHASE}] wrote certificate-ledger poison sweep: {OUT / 'phase47_certificate_ledger_poison_sweep.csv'}")
