# geomlang_phase26dx_lite_culprit_gated_capture_lift_cuda_basic32_E_drive.py
# Phase 26DX-LITE — culprit-gated capture lift after DW.
#
# Why this exists:
...
#
#   DX-LITE therefore does not globally lift the bowl. It searches a narrow band and
...
#   python bbit_geomlang/geomlang_phase26dx_lite_culprit_gated_capture_lift_cuda_basic32_E_drive.py --device cuda
#
# Fast smoke:
#   python bbit_geomlang/geomlang_phase26dx_lite_culprit_gated_capture_lift_cuda_basic32_E_drive.py --device cuda --max-variants 8 --seed-count 2
#
# Wider controlled:
#   python bbit_geomlang/geomlang_phase26dx_lite_culprit_gated_capture_lift_cuda_basic32_E_drive.py --device cuda --max-variants 24 --seed-count 3 --audit-repeats 1
#
# Suspicious audit:
#   python bbit_geomlang/geomlang_phase26dx_lite_culprit_gated_capture_lift_cuda_basic32_E_drive.py --device cuda --max-variants 24 --seed-count 3 --audit-repeats 2 --sample-full-tail
...
PHASE = "26DX-LITE"
TITLE = "culprit-gated capture lift after DW"
...
DW_SUMMARY = ROOT / "phase26dw_lite_summary.json"
DW_RUNS = ROOT / "phase26dw_lite_run_summary.csv"
DW_CASES = ROOT / "phase26dw_lite_culprit_cases.csv"
...
# Narrow DX clamp: avoid the old capture-heavy / tangent-chaotic corner.
BOUNDS = {
    "BOWL_RADIUS_FRAC": (0.685, 0.742),
    "BOWL_SEAT_AXIS_GAIN": (0.385, 0.435),
    "BOWL_DIRECTIONAL_BLEND": (0.955, 1.025),
    "BOWL_NORM_CAP_MULT": (1.075, 1.190),
    "BOWL_SHELL_RADIAL_GAIN": (0.135, 0.172),
    "BOWL_TANGENT_KILL": (0.925, 1.045),
}
...
def load_dw_guided_candidates(limit_each: int = 10) -> List[Tuple[str, Dict[str, float], str]]:
    ...
    add_unique(out, seen, "dx_anchor_dw_balanced_lift", FALLBACK_DW_BALANCED, "fallback_dw_balanced")
    add_unique(out, seen, "dx_anchor_dv_best_grid", FALLBACK_DV_BEST, "fallback_dv_best")
    add_unique(out, seen, "dx_anchor_low_capture_clean", FALLBACK_LOW_CAPTURE_CLEAN, "fallback_low_capture_clean")

    for p in (DW_SUMMARY, DV_SUMMARY):
    ...
    if DW_RUNS.exists():
        try:
            ...
            if not df.empty:
                ...
                df = df.assign(dx_rank=rank).sort_values("dx_rank", ascending=False)
                for _, row in df.head(limit_each).iterrows():
                    ov = parse_overrides_from_row(row)
                    if ov is not None:
                        add_unique(out, seen, f"prior_dw_run_{row.get('audit_variant', 'row')}", ov, str(DW_RUNS))
        except Exception as e:
        ...
        try:
            ...
            if not df.empty:
                ...
                if not safe_low_capture.empty:
                    ...
                    for _, row in safe_low_capture.sort_values("dx_case_rank", ascending=False).head(limit_each).iterrows():
                        ov = parse_overrides_from_row(row)
                        if ov is not None:
                            add_unique(out, seen, f"prior_clean_lowcap_{row.get('envelope_label', 'case')}", ov, str(DW_CASES))
                ...
                if not clean_pass.empty:
                    clean_pass["dx_case_rank"] = (
                        10.0 * clean_pass["capture_rate"].astype(float)
                        + clean_pass["dp_score"].astype(float)
                        - 0.5 * clean_pass["tangent_ratio"].astype(float)
                    )
                    for _, row in clean_pass.sort_values("dx_case_rank", ascending=False).head(limit_each).iterrows():
                        ov = parse_overrides_from_row(row)
                        if ov is not None:
                            add_unique(out, seen, f"prior_clean_pass_{row.get('envelope_label', 'case')}", ov, str(DW_CASES))
        except Exception as e:
    ...
    if DV_VARIANTS.exists():
        try:
            ...
            if not df.empty:
                ...
                df = df.assign(dx_dv_rank=rank).sort_values("dx_dv_rank", ascending=False)
                for _, row in df.head(limit_each).iterrows():
                    ov = parse_overrides_from_row(row)
                    if ov is not None:
...
def generate_dx_variants(seed: int, max_variants: int) -> List[Tuple[str, Dict[str, float], str]]:
    rng = random.Random(seed)
    out: List[Tuple[str, Dict[str, float], str]] = []
    seen: set = set()

    priors = load_dw_guided_candidates(limit_each=8)
    for name, ov, src in priors[:10]:
        add_unique(out, seen, name, ov, src)

    # Hand-picked two-zone compromises: small capture nudge, stronger tangent kill, no high-seat/high-shell corner.
    centers = [FALLBACK_DW_BALANCED, FALLBACK_LOW_CAPTURE_CLEAN, FALLBACK_DV_BEST]
    for ci, center in enumerate(centers):
        for r in [0.705, 0.715, 0.725, 0.735]:
            for s in [0.395, 0.405, 0.415, 0.425]:
                for b in [0.975, 0.990, 1.005, 1.015]:
                    for c in [1.10, 1.13, 1.16, 1.18]:
                        for sh in [0.145, 0.155, 0.162, 0.168]:
                            for tk in [0.96, 0.985, 1.00, 1.025]:
                                # Cheap prefilter against known dangerous corner.
                                if s > 0.425 and sh > 0.165:
                                    continue
                                if b > 1.015 and sh > 0.165:
                                    continue
                                ov = set_ov(center, BOWL_RADIUS_FRAC=r, BOWL_SEAT_AXIS_GAIN=s, BOWL_DIRECTIONAL_BLEND=b,
                                            BOWL_NORM_CAP_MULT=c, BOWL_SHELL_RADIAL_GAIN=sh, BOWL_TANGENT_KILL=tk)
                                # Add a deterministic but sparse set.
                                if rng.random() < 0.055:
                                    add_unique(out, seen, f"dx_grid_c{ci}_r{r:.3f}_s{s:.3f}_b{b:.3f}_c{c:.2f}_sh{sh:.3f}_tk{tk:.3f}", ov, "dx_sparse_grid")

    # Focused line searches. These are the most interpretable mutations.
    for cap in [1.105, 1.125, 1.145, 1.165, 1.185]:
        for tk in [0.975, 1.000, 1.025, 1.040]:
            ov = set_ov(FALLBACK_DW_BALANCED, BOWL_NORM_CAP_MULT=cap, BOWL_TANGENT_KILL=tk,
                        BOWL_SEAT_AXIS_GAIN=0.405, BOWL_SHELL_RADIAL_GAIN=0.155, BOWL_DIRECTIONAL_BLEND=0.990)
            add_unique(out, seen, f"dx_balanced_cap{cap:.3f}_tk{tk:.3f}", ov, "dx_line_balanced")

    for radius in [0.715, 0.725, 0.735]:
        for cap in [1.155, 1.170, 1.185]:
            ov = set_ov(FALLBACK_LOW_CAPTURE_CLEAN, BOWL_RADIUS_FRAC=radius, BOWL_NORM_CAP_MULT=cap,
                        BOWL_SEAT_AXIS_GAIN=0.405, BOWL_SHELL_RADIAL_GAIN=0.162, BOWL_TANGENT_KILL=1.015,
                        BOWL_DIRECTIONAL_BLEND=0.998)
            add_unique(out, seen, f"dx_clean_radius{radius:.3f}_cap{cap:.3f}", ov, "dx_line_lowcap_clean")

    # Gaussian cloud centered between the balanced and low-capture-clean anchors.
    for i in range(max(24, max_variants * 2)):
        ov = {
            "BOWL_RADIUS_FRAC": rng.gauss(0.721, 0.010),
            "BOWL_SEAT_AXIS_GAIN": rng.gauss(0.407, 0.011),
            "BOWL_DIRECTIONAL_BLEND": rng.gauss(0.995, 0.012),
            "BOWL_NORM_CAP_MULT": rng.gauss(1.155, 0.020),
            "BOWL_SHELL_RADIAL_GAIN": rng.gauss(0.160, 0.006),
            "BOWL_TANGENT_KILL": rng.gauss(1.010, 0.020),
        }
        add_unique(out, seen, f"dx_culprit_gate_cloud_{i:03d}", ov, "dx_gaussian_cloud")

    if len(out) > max_variants:
        anchors = out[: min(6, len(out))]
...
def add_culprit_gates(results_df: pd.DataFrame, case_df: pd.DataFrame, target_cap: float, target_tan: float, relaxed_tan: float, target_q20: float) -> pd.DataFrame:
    ...
    for _, row in results_df.iterrows():
        ...
        # DX score directly encodes the DW diagnosis.
        ...
        d["dx_culprit_gated_score_run"] = float(dx_score)
        rows.append(d)
...
def summarize(results_df: pd.DataFrame, target_cap: float, target_tan: float, relaxed_tan: float, target_q20: float) -> pd.DataFrame:
    ...
    for variant, g in results_df.groupby("variant", sort=False):
        ...
        mean_score = float(g["dx_culprit_gated_score_run"].mean())
        ...
        rows.append({
            ...
            "dx_family_score": float(family_score),
            "mean_dx_run_score": mean_score,
    ...
    return pd.DataFrame(rows).sort_values(
        ["stable_named_gate_strict_all_seeds", "stable_named_gate_relaxed_all_seeds", "named_gate_relaxed_rate", "relaxed_pass_rate", "dx_family_score"],
...
def plot_family_scores(df: pd.DataFrame, out_path: Path) -> None:
    if df.empty:
        return
    top = df.sort_values("dx_family_score", ascending=False).head(25).iloc[::-1]
    plt.figure(figsize=(14, max(6, 0.46 * len(top))))
    plt.barh(top["variant"], top["dx_family_score"])
    plt.xlabel("DX culprit-gated family score")
    plt.title("26DX-LITE culprit-gated family scores")
    plt.tight_layout()
...
def plot_pass_rates(df: pd.DataFrame, out_path: Path) -> None:
    if df.empty:
        return
    top = df.sort_values(["named_gate_relaxed_rate", "relaxed_pass_rate", "dx_family_score"], ascending=[False, False, False]).head(25).iloc[::-1]
    y = np.arange(len(top))
    plt.figure(figsize=(14, max(6, 0.46 * len(top))))
    ...
    plt.title("26DX-LITE pass/gate rates")
    plt.legend()
    plt.tight_layout()
    plt.savefig(out_path, dpi=140)
    plt.close()
...
def plot_capture_tangent(df: pd.DataFrame, out_path: Path, target_cap: float, target_tan: float, relaxed_tan: float) -> None:
    ...
    sc = plt.scatter(df["label_min_screen_capture"], df["label_max_guard_tangent"], c=df["dx_culprit_gated_score_run"], s=76, alpha=0.80)
    ...
    plt.colorbar(sc, label="DX gated run score")
    plt.xlabel("min named screen capture")
    plt.ylabel("max named guard tangent")
    plt.title("26DX-LITE named gates: capture vs tangent")
...
def plot_label_culprits(case_df: pd.DataFrame, out_path: Path, target_cap: float, target_tan: float) -> None:
    ...
    plt.title("26DX-LITE remaining culprit labels")
...
def run(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    variants = generate_dx_variants(args.seed, args.max_variants)
    print(f"[{PHASE}] DX variants: {len(variants)}")
    ...
    results_df.to_csv(ROOT / "phase26dx_lite_culprit_gated_results.csv", index=False)
    case_df.to_csv(ROOT / "phase26dx_lite_culprit_gated_cases.csv", index=False)
    summary_df.to_csv(ROOT / "phase26dx_lite_variant_summary.csv", index=False)

    plot_family_scores(summary_df, ROOT / "phase26dx_lite_family_scores.png")
    plot_pass_rates(summary_df, ROOT / "phase26dx_lite_pass_rates.png")
    plot_capture_tangent(results_df, ROOT / "phase26dx_lite_named_capture_vs_tangent.png", target_cap, target_tan, relaxed_tan)
    plot_label_culprits(case_df, ROOT / "phase26dx_lite_remaining_culprit_labels.png", target_cap, target_tan)
    ...
    if len(stable_named_strict) and len(stable_global_strict):
        ...
        verdict = "no_culprit_gated_family_found"
    ...
    if not case_df.empty:
        ...
        label_summary.to_csv(ROOT / "phase26dx_lite_label_summary.csv", index=False)
    ...
    summary = {
        ...
        "best_dx_family_score": None if not len(best) else fnum(best.get("dx_family_score")),
        ...
        "outputs": [
            "phase26dx_lite_culprit_gated_results.csv",
            "phase26dx_lite_culprit_gated_cases.csv",
            "phase26dx_lite_variant_summary.csv",
            "phase26dx_lite_label_summary.csv",
            "phase26dx_lite_summary.json",
            "phase26dx_lite_family_scores.png",
            "phase26dx_lite_pass_rates.png",
            "phase26dx_lite_named_capture_vs_tangent.png",
            "phase26dx_lite_remaining_culprit_labels.png",
        ],
    }
    (ROOT / "phase26dx_lite_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")

    print(f"[{PHASE}] Done in {summary['elapsed_sec']:.1f}s")
    print(f"[{PHASE}] VERDICT: {verdict}")
    if len(best):
        print(
            f"[{PHASE}] BEST VARIANT: {best['variant']} | family={best['dx_family_score']:.3f} "
    ...
    print(f"[{PHASE}] Wrote summary: {ROOT / 'phase26dx_lite_summary.json'}")
    return summary
