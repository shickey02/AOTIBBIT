...
# -*- coding: utf-8 -*-
r"""
Phase 60: Recursive adversarial audit counterexample-forge tribunal

Drop-in path:
  E:\BBIT\bbit_geomlang\geomlang_phase60_counterfactual_recursive_adversarial_audit_counterexample_forge_tribunal_basic32_E_drive.py

Run:
  python bbit_geomlang/geomlang_phase60_counterfactual_recursive_adversarial_audit_counterexample_forge_tribunal_basic32_E_drive.py

Purpose
-------
Phase 51 proved the typed contradiction tribunal could accept the true A -> B -> C certificate,
reject the false certificates, and pass one sealed holdout/null-ablation checks. Phase 60 raises the bar by
...
Phase 60 adds a metamorphic-shadow invariance tribunal on top of the Phase 59 null-ablation near-miss tribunal.
...
Writes into E:\BBIT\outputs_basic32 by default:
  ...
  phase60_recursive_adversarial_audit_counterexample_forge_tribunal_trials.csv
  phase60_recursive_adversarial_audit_counterexample_forge_tribunal_summary.json
  phase60_recursive_adversarial_audit_counterexample_forge_tribunal_report.md
  phase60_recursive_adversarial_audit_counterexample_forge_tribunal_poison_sweep.csv
  phase60_examples\*.png
  phase60_recursive_adversarial_audit_counterexample_forge_tribunal_accuracy.png
  phase60_recursive_adversarial_audit_counterexample_forge_tribunal_rejection_rates.png
  phase60_recursive_adversarial_audit_counterexample_forge_tribunal_false_certificate_score_distribution.png
  phase60_recursive_adversarial_audit_counterexample_forge_tribunal_true_vs_false_score_margin.png
  phase60_recursive_adversarial_audit_counterexample_forge_tribunal_branch_counts.png
  phase60_recursive_adversarial_audit_counterexample_forge_tribunal_poison_sweep.png
  phase60_recursive_adversarial_audit_counterexample_forge_tribunal_forge_rates.png
...
try:
    ...
    plt = None
    print("[60] WARNING: matplotlib unavailable; png plots will be skipped:", exc)


PHASE = "60"
TITLE = "Recursive adversarial audit counterexample-forge tribunal"
PASS_FLAG = "PHASE60_RECURSIVE_ADVERSARIAL_AUDIT_COUNTEREXAMPLE_FORGE_TRIBUNAL_PASS"
...
def run_trial(
    ...
    # Phase 60 calibration layer.  Phase 59 proved that false certificates can be
    ...
    # Phase 60: metamorphic-shadow invariance tribunal.
    ...
    # lightweight: Phase 60 should not multiply the full certificate grid again; it
    ...
    # Phase 60: metamorphic-shadow invariance tribunal.
    # Phase 59 already proved the proof does not collapse when a single score cue
    # is ablated.  Phase 60 asks a harder structural question: does the same
    ...
    return {
        ...
        # Phase 60: recursive adversarial audit. This is a compact skeptical
        # meta-tribunal layered over the Phase 59 minimax panel. It asks whether
        # the winning proof still has room after progressively tightening the
        # margin budget and after the closest false certificate is artificially
        # moved toward the true certificate. The minimum of these compressed
        # margins is recorded as the recursive audit margin.
        "recursive_audit_pass": int(
            calibrated_false_rejected
            and double_sealed_pass
            and jackknife_shadow_pass
            and cross_exam_pass
            ...
            and pair_consensus_rate >= 0.999
            and min(
                0.65 * (best_false.score - winning.score),
                0.80 * near_miss_margin,
                0.90 * log_false_margin,
                0.80 * relative_false_margin,
                jackknife_margin_floor,
                bootstrap_margin_floor,
                pair_margin_floor,
            ) > 0.0005
        ),
        "recursive_audit_margin": float(min(
            0.65 * (best_false.score - winning.score),
            0.80 * near_miss_margin,
            0.90 * log_false_margin,
            0.80 * relative_false_margin,
        ...
        "recursive_audit_pressure": float((best_false.score + near_miss_false_score) / (2.0 * (winning.score + 1e-9))),
...
def summarize(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    recursive_audit_rate = float(np.mean([r["recursive_audit_pass"] for r in rows]))
    recursive_audit_margin_floor = float(min([r["recursive_audit_margin"] for r in rows]))
    mean_recursive_audit_margin = float(np.mean([r["recursive_audit_margin"] for r in rows]))
    mean_recursive_audit_pressure = float(np.mean([r["recursive_audit_pressure"] for r in rows]))
    # Phase 60 adds a jackknife/bootstrap shadow jury: the certificate must remain good when
    ...
    for fam in FAMILIES:
        ...
        fams[fam] = {
            ...
            "recursive_audit_rate": float(np.mean([r["recursive_audit_pass"] for r in sub])),
            "mean_recursive_audit_margin": float(np.mean([r["recursive_audit_margin"] for r in sub])),
            "null_ablation_jury_rate": float(np.mean([1.0 if (r["trajectory_correct"] and r["calibrated_false_rejected"] and r["double_sealed_pass"] and r["jackknife_shadow_pass"] and r["cross_exam_pass"] and r["metamorphic_invariance_pass"] and r["counterexample_forge_pass"] and r["adversarial_minimax_pass"] and r["recursive_audit_pass"] and r["near_miss_margin"] > 0.01 and r["relative_false_margin"] > 0.01 and r["log_false_margin"] > 0.01) else 0.0 for r in sub])),
    ...
    passed = (
        ...
        and recursive_audit_rate >= 0.98
        and recursive_audit_margin_floor > 0.0005
    ...
    return {
        ...
        "recursive_audit_rate": recursive_audit_rate,
        "recursive_audit_margin_floor": recursive_audit_margin_floor,
        "mean_recursive_audit_margin": mean_recursive_audit_margin,
        "mean_recursive_audit_pressure": mean_recursive_audit_pressure,
...
def write_report(path: Path, summary: Dict[str, Any]) -> None:
    ...
    lines.append("")
    lines.append(
        "Phase 60 is a falsification phase. Earlier phases proved that the same branch can survive "
...
def plot_outputs(out_dir: Path, rows: List[Dict[str, Any]], summary: Dict[str, Any]) -> None:
    ...
    plt.ylim(0, 1.08)
    plt.ylabel("rate")
    plt.title("Phase 60 final-only vs true-certificate acceptance and false-certificate rejection")
    plt.legend()
    savefig("phase60_recursive_adversarial_audit_counterexample_forge_tribunal_accuracy.png")
    ...
    plt.title("Phase 60 closest false certificate type")
    savefig("phase60_recursive_adversarial_audit_counterexample_forge_tribunal_rejection_rates.png")
    ...
    plt.ylabel("count")
    plt.title("Phase 60 false certificate score distribution")
    savefig("phase60_recursive_adversarial_audit_counterexample_forge_tribunal_false_certificate_score_distribution.png")
    ...
    plt.ylabel("trials")
    plt.title("Phase 60 true-vs-false certificate selectivity margin")
    savefig("phase60_recursive_adversarial_audit_counterexample_forge_tribunal_true_vs_false_score_margin.png")
    ...
    plt.ylabel("trials")
    plt.title("Phase 60 calibrated near-miss selectivity margin")
    savefig("phase60_recursive_adversarial_audit_counterexample_forge_tribunal_near_miss_margin.png")
    ...
    plt.ylabel("count")
    plt.title("Phase 60 log-scaled false certificate score distribution")
    savefig("phase60_recursive_adversarial_audit_counterexample_forge_tribunal_false_certificate_log_score_distribution.png")
    ...
    plt.title("Phase 60 sealed holdout tribunal pass rate by family")
    savefig("phase60_recursive_adversarial_audit_counterexample_forge_tribunal_holdout_pass_rate.png")
    ...
    plt.xticks(rotation=25, ha="right")
    plt.ylabel("trials")
    plt.title("Phase 60 predicted branch counts")
    savefig("phase60_recursive_adversarial_audit_counterexample_forge_tribunal_branch_counts.png")

    # Phase 60 cross-exam rates: jackknife, bootstrap, and pair panels.
    ...
    plt.ylim(0, 1.08)
    plt.ylabel("rate")
    plt.title("Phase 60 jackknife/bootstrap/pair cross-exam consensus")
    plt.legend()
    savefig("phase60_recursive_adversarial_audit_counterexample_forge_tribunal_cross_exam_rates.png")
...
def poison_sweep(args: argparse.Namespace, out_dir: Path) -> List[Dict[str, Any]]:
    ...
    write_csv(out_dir / "phase60_recursive_adversarial_audit_counterexample_forge_tribunal_poison_sweep.csv", sweep_rows)
    ...
    if plt is not None:
        ...
        plt.ylabel("rate")
        plt.ylim(0, 1.08)
        plt.title("Phase 60 poisoned-witness stress sweep")
        plt.legend()
        plt.tight_layout()
        plt.savefig(out_dir / "phase60_recursive_adversarial_audit_counterexample_forge_tribunal_poison_sweep.png", dpi=150)
        plt.close()
...
def write_examples(out_dir: Path, rows: List[Dict[str, Any]]) -> None:
    if plt is None:
        return
    ex_dir = out_dir / "phase60_examples"
...
def main() -> None:
    ...
    print(f"[{PHASE}] reset continued: from sealed holdout contradiction tribunal to recursive adversarial audit counterexample-forge tribunal")
    print(f"[{PHASE}] task: accept the true A->B->C certificate while rejecting false certificates through typed A/B/C role signatures, axis-aware transition probes, contradiction margins, and a recursive adversarial audit counterexample-forge tribunal that removes each witness in turn, reselects its quorum from the remaining evidence, replays the branch decision, performs bootstrap and pair cross-examination, then forges fresh counterexample certificates from role swaps, endpoint shortcuts, mirror inversions, temporal scrambles, and cross-family impostors; the proof is rejected if it survives only because the false cases were too easy, too familiar, or because a single adversarial false certificate can become the minimax winner under any allowed shadow presentation, or because the proof cannot survive a recursive tightening audit that repeatedly compresses margins, removes its strongest-looking witness, and replays the skeptic panel against the closest false certificate")
    ...
    trials_path = out_dir / "phase60_recursive_adversarial_audit_counterexample_forge_tribunal_trials.csv"
    summary_path = out_dir / "phase60_recursive_adversarial_audit_counterexample_forge_tribunal_summary.json"
    report_path = out_dir / "phase60_recursive_adversarial_audit_counterexample_forge_tribunal_report.md"
    ...
    print(
        ...
        f"recursive_audit_rate={summary['recursive_audit_rate']:.4f} "
    ...
    for fam, s in summary["families"].items():
        print(
            ...
            f"rec_audit={s['recursive_audit_rate']:.3f} "
            ...
            f"rec_margin={s['mean_recursive_audit_margin']:.6f} "
    ...
    print(f"[{PHASE}] wrote contradiction-falsifier poison sweep: {out_dir / 'phase60_recursive_adversarial_audit_counterexample_forge_tribunal_poison_sweep.csv'}")
    print(f"[{PHASE}] wrote example png dir: {out_dir / 'phase60_examples'}")
