...
# -*- coding: utf-8 -*-
"""
Phase 51: Counterfactual sealed-holdout axis-typed contradiction tribunal

Drop-in path:
  E:\BBIT\bbit_geomlang\geomlang_phase51_counterfactual_sealed_holdout_axis_typed_contradiction_tribunal_basic32_E_drive.py

Run:
  python bbit_geomlang/geomlang_phase51_counterfactual_sealed_holdout_axis_typed_contradiction_tribunal_basic32_E_drive.py

Purpose
-------
Phase 50 proved the certificate-ledger path can remain stable under witness deletion,
minimal quorums, hostile coalitions, and cross-examination. But the Phase 50 run also
showed a practical warning sign: the reported metrics were nearly identical to Phase 47,
and the console label still printed [47]. That means the next real step should not be
another renamed audit layer.

Phase 51 adds a sealed-holdout contradiction tribunal on top of the Phase 50 typed falsifier.
...
Writes into E:\BBIT\outputs_basic32 by default:
  ...
  phase51_sealed_holdout_axis_typed_contradiction_tribunal_trials.csv
  phase51_sealed_holdout_axis_typed_contradiction_tribunal_summary.json
  phase51_sealed_holdout_axis_typed_contradiction_tribunal_report.md
  phase51_sealed_holdout_axis_typed_contradiction_tribunal_poison_sweep.csv
  phase51_examples\*.png
  phase51_sealed_holdout_axis_typed_contradiction_tribunal_accuracy.png
  phase51_sealed_holdout_axis_typed_contradiction_tribunal_rejection_rates.png
  phase51_sealed_holdout_axis_typed_contradiction_tribunal_false_certificate_score_distribution.png
  phase51_sealed_holdout_axis_typed_contradiction_tribunal_true_vs_false_score_margin.png
  phase51_sealed_holdout_axis_typed_contradiction_tribunal_branch_counts.png
  phase51_sealed_holdout_axis_typed_contradiction_tribunal_poison_sweep.png
...
try:
    ...
    plt = None
    print("[51] WARNING: matplotlib unavailable; png plots will be skipped:", exc)


PHASE = "51"
TITLE = "Counterfactual sealed-holdout axis-typed contradiction tribunal"
PASS_FLAG = "PHASE51_COUNTERFACTUAL_SEALED_HOLDOUT_AXIS_TYPED_CONTRADICTION_TRIBUNAL_PASS"
...
def write_report(path: Path, summary: Dict[str, Any]) -> None:
    ...
    lines.append("")
    lines.append("| family | traj_acc | final_acc | stable | false_reject | false_margin | ledger_tight | min_margin |")
    lines.append("|---|---:|---:|---:|---:|---:|---:|---:|")
    ...
    lines.append("")
    lines.append(
        "Phase 51 is a falsification phase. Earlier phases proved that the same branch can survive "
...
def plot_outputs(out_dir: Path, rows: List[Dict[str, Any]], summary: Dict[str, Any]) -> None:
    ...
    plt.ylim(0, 1.08)
    plt.ylabel("rate")
    plt.title("Phase 51 final-only vs true-certificate acceptance and false-certificate rejection")
    plt.legend()
    savefig("phase51_sealed_holdout_axis_typed_contradiction_tribunal_accuracy.png")
    ...
    plt.title("Phase 51 closest false certificate type")
    savefig("phase51_sealed_holdout_axis_typed_contradiction_tribunal_rejection_rates.png")
    ...
    plt.ylabel("count")
    plt.title("Phase 51 false certificate score distribution")
    savefig("phase51_sealed_holdout_axis_typed_contradiction_tribunal_false_certificate_score_distribution.png")
    ...
    plt.ylabel("trials")
    plt.title("Phase 51 true-vs-false certificate selectivity margin")
    savefig("phase51_sealed_holdout_axis_typed_contradiction_tribunal_true_vs_false_score_margin.png")
    ...
    plt.title("Phase 51 sealed holdout tribunal pass rate by family")
    savefig("phase51_sealed_holdout_axis_typed_contradiction_tribunal_holdout_pass_rate.png")
    ...
    plt.xticks(rotation=25, ha="right")
    plt.ylabel("trials")
    plt.title("Phase 51 predicted branch counts")
    savefig("phase51_sealed_holdout_axis_typed_contradiction_tribunal_branch_counts.png")
...
def poison_sweep(args: argparse.Namespace, out_dir: Path) -> List[Dict[str, Any]]:
    ...
    write_csv(out_dir / "phase51_sealed_holdout_axis_typed_contradiction_tribunal_poison_sweep.csv", sweep_rows)
    ...
    if plt is not None:
        ...
        plt.ylabel("rate")
        plt.ylim(0, 1.08)
        plt.title("Phase 51 poisoned-witness stress sweep")
        plt.legend()
        plt.tight_layout()
        plt.savefig(out_dir / "phase51_sealed_holdout_axis_typed_contradiction_tribunal_poison_sweep.png", dpi=150)
        plt.close()
...
def write_examples(out_dir: Path, rows: List[Dict[str, Any]]) -> None:
    if plt is None:
        return
    ex_dir = out_dir / "phase51_examples"
    ...
    for r in selected:
        ...
        plt.tight_layout()
        plt.savefig(ex_dir / f"phase51_trial_{int(r['trial']):04d}_certificate_scores.png", dpi=140)
        plt.close()
...
def main() -> None:
    ...
    print(f"[{PHASE}] reset continued: from axis-typed contradiction falsification to sealed holdout contradiction tribunal")
    print(f"[{PHASE}] task: accept the true A->B->C certificate while rejecting false certificates through typed A/B/C role signatures, axis-aware transition probes, contradiction margins, and a sealed holdout consistency tribunal")
    ...
    trials_path = out_dir / "phase51_sealed_holdout_axis_typed_contradiction_tribunal_trials.csv"
    summary_path = out_dir / "phase51_sealed_holdout_axis_typed_contradiction_tribunal_summary.json"
    report_path = out_dir / "phase51_sealed_holdout_axis_typed_contradiction_tribunal_report.md"
    ...
    print(f"[{PHASE}] wrote contradiction-falsifier poison sweep: {out_dir / 'phase51_sealed_holdout_axis_typed_contradiction_tribunal_poison_sweep.csv'}")
    print(f"[{PHASE}] wrote example png dir: {out_dir / 'phase51_examples'}")
