...
# -*- coding: utf-8 -*-
r"""
Phase 40: Counterfactual twin-context causal abstraction reasoner
...
Phase 40 adds a harder witness problem: the system now receives several source
...
Outputs:
  E:\BBIT\outputs_basic32\phase40_twin_context_abstraction_trials.csv
  E:\BBIT\outputs_basic32\phase40_twin_context_abstraction_summary.json
  E:\BBIT\outputs_basic32\phase40_twin_context_abstraction_report.md
  E:\BBIT\outputs_basic32\phase40_twin_poison_sweep.csv
  E:\BBIT\outputs_basic32\phase40_examples\*.png
...
PHASE = "40"
TITLE = "Counterfactual twin-context causal abstraction reasoner"
...
EXAMPLE_DIR = OUT / "phase40_examples"
...
def make_twin_candidates(
    ...
    # Looks good in the first target context, breaks under the twin-context replay.
...
def plot_accuracy(summary: Dict[str, object]) -> None:
    ...
    ax.set_title("Phase 40 counterfactual twin-context causal abstraction reasoning: accuracy by hidden family")
    for i, v in enumerate(vals):
        ax.text(v + 0.01, i, f"{v:.2f}", va="center")
    fig.tight_layout()
    fig.savefig(OUT / "phase40_twin_context_abstraction_accuracy.png", dpi=140)
    plt.close(fig)
...
def plot_baseline_gap(summary: Dict[str, object]) -> None:
    ...
    ax.bar(x + width/2, traj, width, label="twin-context abstraction")
    ax.set_ylim(0, 1.05)
    ax.set_ylabel("accuracy")
    ax.set_title("Phase 40 final-only baseline vs twin-context causal abstraction")
    ax.set_xticks(x)
    ax.set_xticklabels(labels, rotation=22, ha="right")
    ax.legend()
    fig.tight_layout()
    fig.savefig(OUT / "phase40_twin_context_abstraction_baseline_gap.png", dpi=140)
    plt.close(fig)
...
def plot_branch_counts(rows: List[Dict[str, object]]) -> None:
    ...
    ax.bar(labels, vals)
    ax.set_ylabel("trials")
    ax.set_title("Phase 40 predicted branch counts")
    ax.tick_params(axis="x", rotation=25)
    fig.tight_layout()
    fig.savefig(OUT / "phase40_twin_context_abstraction_branch_counts.png", dpi=140)
    plt.close(fig)
...
def write_poison_sweep(rows: List[Dict[str, float]]) -> None:
    path = OUT / "phase40_twin_poison_sweep.csv"
    with path.open("w", newline="", encoding="utf-8") as f:
    ...
    ax.set_ylabel("accuracy")
    ax.set_title("Phase 40 poisoned-witness stress sweep")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(OUT / "phase40_twin_context_abstraction_poison_sweep.png", dpi=140)
    plt.close(fig)
...
def write_report(summary: Dict[str, object], path: Path) -> None:
    ...
    lines.append("\n## Interpretation\n")
    lines.append(
        "Phase 40 tests whether the path representation can become a consensus object. "
...
def save_examples(rng: np.random.Generator, n: int = 4) -> None:
    ...
    for i, fam in enumerate(FAMILIES[:n]):
        ...
        axes[2].set_title("twin-context rejection")
        axes[2].axis("equal")
...
def main() -> None:
    ...
    print(f"[{PHASE}] reset continued: from cross-context bridge reasoning to twin-context causal abstraction")
    print(f"[{PHASE}] task: infer invariant A->B->C from witnesses, then require the answer branch to survive two target contexts")

    rows = [run_trial(rng, i, poison_fraction=POISON_FRACTION) for i in range(TRIALS)]
    summary = aggregate(rows)

    trials_path = OUT / "phase40_twin_context_abstraction_trials.csv"
    summary_path = OUT / "phase40_twin_context_abstraction_summary.json"
    report_path = OUT / "phase40_twin_context_abstraction_report.md"
    ...
    plot_hist(rows, "margin", "Phase 40 twin-context trajectory answer margin distribution", "runner-up trajectory score - best trajectory score", "phase40_twin_context_abstraction_margin_distribution.png")
    plot_hist(rows, "final_margin", "Phase 40 final-only endpoint margin distribution", "runner-up endpoint score - best endpoint score", "phase40_twin_context_abstraction_final_only_margin_distribution.png")
    sweep_rows = poison_sweep()
    write_poison_sweep(sweep_rows)
    save_examples(rng)

    print(f"[{PHASE}] PHASE40_COUNTERFACTUAL_TWIN_CONTEXT_CAUSAL_ABSTRACTION_PASS={summary['pass']}")
    print(
    ...
    print(f"[{PHASE}] wrote twin poison sweep: {OUT / 'phase40_twin_poison_sweep.csv'}")
